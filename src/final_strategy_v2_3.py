"""
Final Strategy V2.3 - Iteration 24: 动态进攻与利润跑腾增强

【核心改进自 V2.2】
1. 动态滚动百分位排名 - 废弃静态分位数，使用 Polars .rank(percentile=True)
2. 线性权重映射 - ridge_weight = 0.2 + (0.6 * atr_rank), 范围 0.2~0.8
3. 利润跑腾逻辑升级 - 多级放松 + 信号锁定 (盈利>5% 时 100% LGB)
4. 因子增强 - vol_price_resilience 除零保护 + relative_strength_sector 新因子

【V2.2 问题诊断】
- 权重切换次数为 0：静态分位数边界导致所有日期判定为同一波动 regime
- 利润跑腾逻辑单一：只有 3% 一级放松，无法适应不同盈利阶段
- 因子计算风险：vol_price_resilience 缺少除零保护

【解决方案】
1. 滚动百分位：计算当前标的 ATR 在过去 250 日的百分位排名 atr_rank (0.0~1.0)
2. 线性映射：ridge_weight = 0.2 + 0.6 * atr_rank，确保每天都有动态权重
3. 多级放松：
   - 盈利>3%: relax_factor = 1.2
   - 盈利>6%: relax_factor = 1.5 + trailing_stop_buffer 翻倍
4. 信号锁定：盈利>5% 时，预测分数 100% 采用 LightGBM 输出

作者：量化策略团队
版本：Final Strategy V2.3 (Iteration 24)
日期：2026-03-16
"""
...
# ===========================================
# 配置常量 - Iteration 24 更新
# ===========================================

# 【Iteration 24】多模型集成参数
ENSEMBLE_ENABLED = True
RIDGE_WEIGHT = 0.5      # Ridge 基础权重 50%
LGB_WEIGHT = 0.5        # LightGBM 基础权重 50%

# 【Iteration 24】动态权重调整参数 - 线性映射
DYNAMIC_WEIGHT_ENABLED = True
ATR_VOLATILITY_WINDOW = 250  # 滚动窗口扩展至 250 日

# 线性权重映射参数
RIDGE_WEIGHT_MIN = 0.2  # 最低 Ridge 权重 (低波动时)
RIDGE_WEIGHT_MAX = 0.8  # 最高 Ridge 权重 (高波动时)

# 【Iteration 24】利润跑腾逻辑 - 多级放松
TRAILING_STOP_RELAX_ENABLED = True

# 第一级：盈利>3%
TRAILING_STOP_PROFIT_THRESHOLD_L1 = 0.03
TRAILING_STOP_RELAX_FACTOR_L1 = 1.2

# 第二级：盈利>6%
TRAILING_STOP_PROFIT_THRESHOLD_L2 = 0.06
TRAILING_STOP_RELAX_FACTOR_L2 = 1.5
TRAILING_STOP_BUFFER_MULTIPLIER_L2 = 2.0  # buffer 翻倍

# 【Iteration 24】信号锁定 - 盈利>5% 时 100% LGB
SIGNAL_LOCK_ENABLED = True
SIGNAL_LOCK_PROFIT_THRESHOLD = 0.05  # 5% 盈利阈值
...
TRAILING_STOP_BUFFER_HIGH_VOL = 0.005  # V2.3 提高最小 buffer
...
class Position:
    ...
    profit_relaxed: bool = False  # 是否已触发利润跑腾逻辑
    profit_relax_level: int = 0   # V2.3 新增：放松级别 (0=未触发，1=L1, 2=L2)
    signal_locked: bool = False   # V2.3 新增：是否已信号锁定 (100% LGB)
...
class TradeRecord:
    ...
    max_profit_pct: float = 0.0  # V2.3 新增：持仓期间最大盈利比例
    relax_level_triggered: int = 0  # V2.3 新增：触发的放松级别
...
class FinalStrategyV23:
    """
    Final Strategy V2.3 - Iteration 24 动态进攻与利润跑腾增强
    
    核心改进:
        1. 动态滚动百分位排名 - 使用 Polars .rank(percentile=True)
        2. 线性权重映射 - ridge_weight = 0.2 + 0.6 * atr_rank
        3. 利润跑腾多级放松 - 3%/6% 两级放松 + buffer 翻倍
        4. 信号锁定 - 盈利>5% 时 100% 采用 LightGBM
        5. 因子增强 - vol_price_resilience 除零保护 + relative_strength_sector
    """
    ...
    def __init__(
        ...
        # V2.3: 不再使用静态分位数，改用滚动百分位排名
        # self.atr_low_q 和 self.atr_high_q 已废弃
        ...
        # V2.3: ATR 滚动历史 (用于计算百分位排名)
        self.atr_history_dict: Dict[str, List[float]] = {}  # 按标的存储
        ...
        # V2.3 利润跑腾统计
        self.profit_relax_stats = {
            "l1_triggered": 0,  # L1 放松触发次数
            "l2_triggered": 0,  # L2 放松触发次数
            "signal_locked": 0,  # 信号锁定触发次数
        }
        
        logger.info("FinalStrategyV2.3 initialized")
    ...
    def _compute_atr_rank(self, df: pl.DataFrame, symbol: str, current_atr: float, window: int = 250) -> float:
        ...
        try:
            ...
            # 使用 Polars 原生 rank 计算百分位
            # percentile=True 返回 0~1 之间的排名
            ranks = atr_series.rank(method="average", percentile=True)
            
            # 找到当前 ATR 在历史中的位置（最接近的值）
    ...
    def _update_dynamic_weights(self, atr: float, symbol: str = None, df: pl.DataFrame = None) -> Tuple[float, float]:
        """
        【V2.3 修复】动态更新集成权重 - 基于滚动百分位排名和线性映射
        
        【核心公式】
        ridge_weight = 0.2 + (0.6 * atr_rank)
        lgb_weight = 1.0 - ridge_weight
        
        【金融逻辑】
        - atr_rank = 0.0 (最低波动): ridge_weight = 0.2, lgb_weight = 0.8
        - atr_rank = 1.0 (最高波动): ridge_weight = 0.8, lgb_weight = 0.2
        - 线性映射确保每天都有动态权重，告别"切换次数为 0"
        
        Args:
            atr: 当前 ATR 值
            symbol: 股票代码 (用于计算历史排名)
            df: 历史数据 DataFrame
        ...
        # 线性权重映射
        ridge_weight = RIDGE_WEIGHT_MIN + (RIDGE_WEIGHT_MAX - RIDGE_WEIGHT_MIN) * atr_rank
        lgb_weight = 1.0 - ridge_weight
        
        # 确保权重在有效范围内
        ridge_weight = max(RIDGE_WEIGHT_MIN, min(RIDGE_WEIGHT_MAX, ridge_weight))
        lgb_weight = max(0.2, min(0.8, lgb_weight))
    ...
    def _get_dynamic_buffer(self, atr: float, relax_level: int = 0) -> float:
        """
        获取动态 Buffer - V2.3 增加多级放松支持
        
        Args:
            atr: 当前 ATR 值
            relax_level: 放松级别 (0=未触发，1=L1, 2=L2)
        """
        if not DYNAMIC_BUFFER_CONTROL:
            base_buffer = TRAILING_STOP_BUFFER
        else:
            # 简化 buffer 逻辑，使用固定值
            base_buffer = TRAILING_STOP_BUFFER_LOW_VOL
        
        # V2.3 多级放松：L2 级别 buffer 翻倍
        if relax_level >= 2:
            base_buffer *= TRAILING_STOP_BUFFER_MULTIPLIER_L2
    ...
    def predict_ensemble(self, df: pl.DataFrame, atr: float = 0.02, symbol: str = None) -> pl.DataFrame:
        """
        使用集成模型进行预测 - 【V2.3 关键修复】
        
        【修复内容】
        - 使用线性映射 ridge_weight = 0.2 + 0.6 * atr_rank
        - 确保每一跳都使用实时 ATR 排名计算动态权重
        - 支持信号锁定：持仓盈利>5% 时 100% 采用 LGB
        """
        ...
        # V2.3: 检查是否需要信号锁定 (持仓盈利>5% 时 100% LGB)
        if SIGNAL_LOCK_ENABLED and symbol and symbol in self.positions:
            position = self.positions[symbol]
            profit_pct = (position.highest_price - position.entry_price) / (position.entry_price + 1e-6)
            if profit_pct >= SIGNAL_LOCK_PROFIT_THRESHOLD:
                ridge_weight = 0.0
                lgb_weight = 1.0
                position.signal_locked = True
                self.profit_relax_stats["signal_locked"] += 1
                logger.debug(f"[信号锁定] {symbol}: 盈利={profit_pct:.1%} >= {SIGNAL_LOCK_PROFIT_THRESHOLD:.0%}, 100% LGB")
    ...
    def update_trailing_stop(self, symbol: str, position: Position, 
        ...
        【改进内容】
        - 盈利>3%: relax_factor = 1.2 (L1)
        - 盈利>6%: relax_factor = 1.5 + buffer 翻倍 (L2)
        - 信号锁定：盈利>5% 时 100% 采用 LGB 预测
        """
        ...
        # 更新持仓最大盈利记录
        if profit_pct > position.current_score:  # 借用 current_score 暂存最大盈利
            pass  # highest_price 已更新
        ...
        # L1: 盈利>3%
        if TRAILING_STOP_RELAX_ENABLED and profit_pct >= TRAILING_STOP_PROFIT_THRESHOLD_L1:
            relax_factor = TRAILING_STOP_RELAX_FACTOR_L1
            relax_level = 1
            
            # L2: 盈利>6%
            if profit_pct >= TRAILING_STOP_PROFIT_THRESHOLD_L2:
                relax_factor = TRAILING_STOP_RELAX_FACTOR_L2
                relax_level = 2
                
                # 记录 L2 触发
                if position.profit_relax_level < 2:
                    self.profit_relax_stats["l2_triggered"] += 1
                    logger.info(f"[利润跑腾 L2] {symbol}: 盈利={profit_pct:.1%} >= {TRAILING_STOP_PROFIT_THRESHOLD_L2:.0%}, "
                               f"放松系数={relax_factor}, buffer 翻倍")
            elif position.profit_relax_level < 1:
                self.profit_relax_stats["l1_triggered"] += 1
                logger.info(f"[利润跑腾 L1] {symbol}: 盈利={profit_pct:.1%} >= {TRAILING_STOP_PROFIT_THRESHOLD_L1:.0%}, "
                           f"放松系数={relax_factor}")
        
        position.profit_relaxed = relax_level > 0
        position.profit_relax_level = relax_level
        
        # 计算动态 buffer (考虑放松级别)
        dynamic_buffer = self._get_dynamic_buffer(atr, relax_level)
        ...
        if current_price < trailing_stop:
            ...
            if relax_level == 1:
                ...
                relax_info = " (L2 放松+buffer 翻倍)"
            
            logger.info(f"[追踪止盈{relax_info}] {symbol}: 最高价={position.highest_price:.2f}, "
                       f"追踪止盈价={trailing_stop:.2f} (ATR={atr:.3f}*{relax_factor}, buffer={dynamic_buffer:.4f}), "
    ...
    def compute_vol_price_resilience(self, df: pl.DataFrame, lookback: int = 10) -> pl.DataFrame:
        """
        计算价格 - 成交量 Resilience 因子（洗盘结束信号）- 【V2.3 增强除零保护】
    ...
    def compute_relative_strength_sector(self, df: pl.DataFrame, period: int = 20) -> pl.DataFrame:
        """
        【V2.3 新增】计算相对强度因子 (Relative Strength vs Sector)
        ...
        # 限制相对强度在合理范围内
        relative_strength_clipped = relative_strength.clip(0.5, 2.0)
    ...
    def compute_all_enhanced_factors(self, df: pl.DataFrame) -> pl.DataFrame:
        """计算所有增强因子 - 【V2.3 新增 relative_strength_sector】"""
        result = df.clone()
        ...
        result = self.compute_vol_price_resilience(result, lookback=10)  # V2.3 增强除零保护
        result = self.compute_relative_strength_sector(result, period=20)  # V2.3 新增
        return result
    ...
    def _get_financial_logic(self, factor_name: str) -> str:
        logic_map = {
            ...
            "relative_strength_sector": "相对行业强度，筛选领涨品种",  # V2.3 新增
            "return_20d": "20 日收益率",
        }
    ...
    def _get_feature_columns(self) -> List[str]:
        ...
        enhanced_factors = [
            ...
            "relative_strength_sector", "return_20d"  # V2.3 新增
        ]
    ...
    def _exit_position(self, symbol: str, date: str, price: float, reason: str) -> None:
        ...
        self.trade_records.append(TradeRecord(
            ...
            relax_level_triggered=position.profit_relax_level,
        ))
    ...
    def run_attribution_analysis(self) -> Dict[str, Any]:
        if not self.trade_records:
            return {"loss_reasons": {}, "trailing_stop_stats": {}}
        loss_reasons = {}
        ...
        # V2.3: 统计 trailing_stop 触发时的平均盈利
        trailing_stop_stats = {
    ...
    def auto_adjust_params(self, backtest_result: BacktestResult, 
        ...
        # V2.3: 如果 trailing_stop 平均盈利 < 2%，说明止盈太紧
        trailing_stats = attribution_result.get("trailing_stop_stats", {})
        if trailing_stats.get("avg_profit", 0) < 0.02 and trailing_stats.get("count", 0) > 0:
            logger.warning(f"[参数调整建议] trailing_stop 平均盈利={trailing_stats['avg_profit']:.1%} < 2%, 建议调宽止盈")
            adjusted_params["trailing_stop_atr_multiplier"] = TRAILING_STOP_ATR_MULTIPLIER * 1.1
    ...
    def get_weight_trend_analysis(self) -> Dict[str, Any]:
        ...
        # 按月度统计（简化版本，假设 weight_history 按顺序记录）
        monthly_ridge = []
        monthly_lgb = []
        monthly_atr = []
        monthly_rank = []
...
def run_walk_forward_validation(strategy: FinalStrategyV23) -> Dict[str, Any]:
    logger.info("=" * 60)
    ...
    return {
        ...
        "profit_relax_stats": strategy.profit_relax_stats,
    }
...
def generate_audit_report(strategy: FinalStrategyV23, walk_forward_result: Dict[str, Any],
                          stress_result: Dict[str, Any], attribution_result: Dict[str, Any],
                          liquidity_stress_result: Optional[Dict[str, Any]] = None) -> str:
    report = []
    report.append("# Iteration 24 全周期审计报告 (V2.3)")
    report.append("")
    report.append(f"**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"**审计版本**: Final Strategy V2.3 (Iteration 24)")
    report.append("")
    ...
    report.append("")
    report.append("Iteration 24 针对动态进攻与利润跑腾逻辑进行了全面升级:")
    report.append("")
    report.append("1. **动态滚动百分位排名**: 废弃静态分位数，使用 Polars .rank(percentile=True) 计算 ATR 排名")
    report.append("2. **线性权重映射**: ridge_weight = 0.2 + 0.6 * atr_rank, 范围 0.2~0.8")
    report.append("3. **利润跑腾多级放松**: 盈利>3% 放松 1.2 倍，盈利>6% 放松 1.5 倍 + buffer 翻倍")
    report.append("4. **信号锁定**: 盈利>5% 时 100% 采用 LightGBM 预测")
    report.append("5. **因子增强**: vol_price_resilience 除零保护 + relative_strength_sector 新因子")
    report.append("")
    ...
    profit_relax_stats = walk_forward_result.get("profit_relax_stats", {})
    ...
    report.append("### 2.4 利润跑腾统计")
    report.append("")
    report.append("| 指标 | 数值 |")
    report.append("|------|------|")
    report.append(f"| L1 放松触发次数 (盈利>3%) | {profit_relax_stats.get('l1_triggered', 0)} |")
    report.append(f"| L2 放松触发次数 (盈利>6%) | {profit_relax_stats.get('l2_triggered', 0)} |")
    report.append(f"| 信号锁定触发次数 (盈利>5%) | {profit_relax_stats.get('signal_locked', 0)} |")
    report.append("")
    
    report.append("### 2.5 收益对比分析")
    report.append("")
    ...
    if trailing_stats.get('avg_profit', 0) < 0.02 and trailing_stats.get('count', 0) > 0:
        report.append("⚠️ **警告**: trailing_stop 平均盈利 < 2%，建议调宽止盈参数")
    report.append("")
    ...
    report.append("")
    report.append(f"1. **策略有效性**: Iteration 24 在盲测区间 (2024Q1-Q2) 实现 {bt_result.get('total_return', 0):.2%} 收益，")
    if bt_result.get('total_return', 0) > 0.0389:
    ...
    report.append("")
    report.append(f"2. **动态权重**: 平均 Ridge={overall_avg.get('ridge_weight', 0):.1%}, LGB={overall_avg.get('lgb_weight', 0):.1%}")
    report.append("")
    report.append(f"3. **利润跑腾**: L1 触发={profit_relax_stats.get('l1_triggered', 0)}次，L2 触发={profit_relax_stats.get('l2_triggered', 0)}次，信号锁定={profit_relax_stats.get('signal_locked', 0)}次")
    report.append("")
    report.append(f"4. **新增因子**: relative_strength_sector 用于筛选领涨品种")
    report.append("")
    
    report.append("### 6.2 后续优化方向")
    report.append("")
    report.append("1. 继续优化利润跑腾多级放松参数")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Final Strategy V2.3 - Iteration 24")
    logger.info("动态进攻与利润跑腾增强")
    logger.info("=" * 60)
    
    strategy = FinalStrategyV23(
    ...
    report_path = Path("reports/Iteration24_V23_Audit_Report.md")
    ...
    logger.info(f"L1 放松触发：{wf_result.get('profit_relax_stats', {}).get('l1_triggered', 0)}次")
    logger.info(f"L2 放松触发：{wf_result.get('profit_relax_stats', {}).get('l2_triggered', 0)}次")
    logger.info(f"信号锁定触发：{wf_result.get('profit_relax_stats', {}).get('signal_locked', 0)}次")
    logger.info("=" * 60)
