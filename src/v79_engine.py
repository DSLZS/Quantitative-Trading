"""
V79 Backtest Engine - 行业残差 Alpha 与流动性过滤增强回测引擎
...
5. 评分分布直方图生成
6. 数据完整性检查与重试机制
7. V-Shock 2.0 一票否决执行

【V79 新增特性】
- 行业收益实时计算（解决 DB 冲突）
- Residual Alpha 2.0: (个股 5 日 RS - 行业 5 日均值 RS) / 波动率
...
from src.core.v79_logic import (
    V79DataManager,
    V79AlphaCenter,
    V79RankICCalculator,
    V79Signal,
...
class V79BacktestResult:
    ...
    monthly_ic_stats: List[V79MonthlyICStats] = field(default_factory=list)
    ...
    v_shock_filtered_count: int = 0
    
    # 详细数据
    daily_values: Optional[pl.DataFrame] = None
    trades: List[V79Trade] = field(default_factory=list)
    
    # 行业分布统计
    sector_allocation: Dict[str, float] = field(default_factory=dict)
    
    # IC 质量预警
    ic_quality_alerts: List[V79ICQualityAlert] = field(default_factory=list)
...
class V79BacktestEngine:
    """
    V79 回测引擎 - 行业残差 Alpha 与流动性过滤增强
    ...
    def __init__(self, db: Optional[DatabaseManager] = None,
        ...
        # 初始资金（严禁修改）
        self.initial_capital = self.config.get('initial_capital', V79_INITIAL_CAPITAL)
        
        # 仓位管理
        self.max_positions = self.config.get('max_positions', V79_MAX_POSITIONS)
        self.max_single_position_pct = self.config.get(
            'max_single_position_pct', V79_MAX_SINGLE_POSITION_PCT
        )
        self.max_sector_weight = self.config.get(
            'max_sector_weight', V79_MAX_SECTOR_WEIGHT
        )
        
        # 费率（严禁修改）
        self.commission_rate = V79_COMMISSION_RATE
        self.min_commission = V79_MIN_COMMISSION
        self.slippage_buy = V79_SLIPPAGE_BUY
        self.slippage_sell = V79_SLIPPAGE_SELL
        self.stamp_duty = V79_STAMP_DUTY
        self.transfer_fee = V79_TRANSFER_FEE
        
        # 止损止盈（严禁修改）
        self.stop_loss_ratio = V79_STOP_LOSS_RATIO
        self.profit_target_ratio = V79_PROFIT_TARGET_RATIO
        self.trailing_stop_ratio = V79_TRAILING_STOP_RATIO
        
        # 初始化组件
        self.data_manager = V79DataManager(db=self.db, config=self.config)
        self.alpha_center = V79AlphaCenter(config=self.config)
        self.rank_ic_calculator = V79RankICCalculator(db=self.db, config=self.config)
        
        # 回测状态
        self.positions: Dict[str, V79Position] = {}
        self.trades: List[V79Trade] = []
        ...
        self.v_shock_filtered_count = 0
        
        # 行业持仓统计
        self.sector_holdings: Dict[str, float] = {}
        
        # IC 质量预警
        self.ic_quality_alerts: List[V79ICQualityAlert] = []
        
        logger.info(f"V79BacktestEngine 初始化完成：初始资金={self.initial_capital:,.0f}")
    
    def _check_data_integrity(self) -> bool:
        """检查数据完整性"""
        logger.info("=" * 60)
        logger.info("V79: 数据完整性检查")
        logger.info("=" * 60)
        
        is_valid, message = self.data_manager.check_2024_data_integrity()
        
        if not is_valid:
            logger.warning(f"V79: 数据完整性检查未通过 - {message}")
            logger.warning("V79: 建议先运行数据同步脚本补充数据")
        else:
            logger.info(f"V79: 数据完整性检查通过 - {message}")
        
        return is_valid
    
    def run_backtest(self, start_date: str, end_date: str) -> V79BacktestResult:
        """运行回测"""
        logger.info("=" * 60)
        logger.info("V79 回测开始：行业残差 Alpha 与流动性过滤增强")
        logger.info(f"回测区间：[{start_date}, {end_date}]")
        logger.info(f"初始资金：{self.initial_capital:,.0f}")
        logger.info(f"Rank IC 目标：>={V79_RANK_IC_TARGET}")
        logger.info(f"最大回撤目标：<={V79_MAX_DRAWDOWN_TARGET*100:.1f}%")
        logger.info(f"胜率目标：>={V79_WIN_RATE_TARGET*100:.1f}%")
        logger.info("=" * 60)
    ...
    def _execute_trades(self, signals_df: pl.DataFrame,
        ...
        logger.info(f"V79: 回测完成，共 {trading_days} 个交易日")
    ...
    def _execute_buy_with_sector_control(self, signals: List[V79Signal], 
                                          trade_date: str) -> None:
        """执行买入（带行业分散性控制）"""
        if not signals:
            return
        ...
        for signal in sorted_signals:
            ...
            # 统计 V-Shock 过滤
            if signal.v_shock_filtered:
                self.v_shock_filtered_count += 1
    ...
    def _execute_sell(self, symbol: str, position: V79Position,
    ...
    def _compute_backtest_result(self, start_date: str, end_date: str,
                                  ...
                                  score_hist_data: Dict[str, Any]) -> V79BacktestResult:
        """计算回测结果"""
        # 1. 收益指标
        ...
        buy_trade_map: Dict[str, List[V79Trade]] = {}
        ...
        result = V79BacktestResult(
            ...
            v_shock_filtered_count=self.v_shock_filtered_count,
    ...
    def print_backtest_result(self, result: V79BacktestResult) -> None:
        """打印回测结果"""
        logger.info("=" * 60)
        logger.info("V79 回测结果 - 行业残差 Alpha 与流动性过滤增强")
        logger.info("=" * 60)
        ...
        logger.info(f"  V-Shock 过滤数： {result.v_shock_filtered_count}")
        logger.info("-" * 40)
        logger.info("【Rank IC 指标】（核心验收标准）")
        logger.info(f"  Mean Rank IC:      {result.mean_rank_ic:.4f} (目标：>{V79_RANK_IC_TARGET})")
        logger.info(f"  月度 Rank IC 均值：  {result.monthly_rank_ic:.4f} (目标：>{V79_RANK_IC_TARGET})")
        ...
        logger.info(f"  指标 D - 交易日 >= 242: {'✓' if result.trading_days >= 242 else '✗'} ({result.trading_days}天)")
        logger.info("=" * 60)
    
    def generate_report(self, result: V79BacktestResult, 
        ...
        if output_path is None:
            ...
            output_path = output_dir / f"v79_backtest_report_{timestamp}.md"
        ...
        report_lines = [
            ...
            f"| V-Shock 过滤数 | {result.v_shock_filtered_count} |",
            "",
            "## Rank IC 指标（验收标准 A）",
            "",
            "| 指标 | 值 | 目标 | 达标 |",
            "|------|-----|------|------|",
            f"| Mean Rank IC | {result.mean_rank_ic:.4f} | >{V79_RANK_IC_TARGET} | {'✓' if rank_ic_pass else '✗'} |",
            f"| 月度 Rank IC 均值 | {result.monthly_rank_ic:.4f} | >{V79_RANK_IC_TARGET} | {'✓' if result.rank_ic_pass else '✗'} |",
        ...
        report_lines.extend([
            ...
            "| 指标 | 目标 | 实际值 | 达标 |",
            "|------|------|--------|------|",
            f"| 指标 A - Mean Rank IC | >= 0.035 | {result.mean_rank_ic:.4f} | {'✓' if rank_ic_pass else '✗'} |",
            f"| 指标 B - 最大回撤 | <= 10% | {result.max_drawdown*100:.2f}% | {'✓' if drawdown_pass else '✗'} |",
            f"| 指标 C - 胜率 | >= 45% | {result.win_rate*100:.2f}% | {'✓' if win_rate_pass else '✗'} |",
            f"| 指标 D - 交易日 | >= 242 | {result.trading_days} | {'✓' if trading_days_pass else '✗'} |",
            "",
            "---",
            "*V79 回测报告完成*",
        ])
...
def run_v79_backtest(start_date: str = "2024-01-01",
                     end_date: str = "2024-12-31",
                     output_path: Optional[str] = None) -> V79BacktestResult:
    """
    运行 V79 回测
    ...
    Returns:
        V79BacktestResult: 回测结果
    """
    # 初始化数据库
    db = DatabaseManager()
    
    # 初始化引擎
    engine = V79BacktestEngine(db=db)
