"""
Final Strategy V2.6 - Iteration 27: 因子库健壮性修复与 Alpha 增强

【核心改进自 V2.5】
1. 因子库健壮性修复 (Factor Robustness Fix)
   - fill_null 在所有表达式最底层执行，彻底消除预测时缺失值报警
   - 增加防御性逻辑：因子缺失值超过 30% 时，赋 0 权重并记录告警日志

2. 领头羊保护参数降噪 (Leader Protection Refinement)
   - 识别阈值：收紧至 Top 3% (从 Top 10%)
   - 保护力度：调整为 2.0x ATR + 1.5% buffer (从 5.0x ATR + 5% buffer)
   - 动态持有期：废除 15 天死限，改为"盈利回撤超过最高点 30% 且持有超过 10 天"则退出

3. ATR Rank 计算修复 (ATR Rank Stabilization)
   - 确保使用过去 250 天滚动百分位排名
   - 添加模式切换平滑机制，避免同一天内模式反复切换

4. 动态 Alpha 因子集 (Dynamic Alpha Factor Set)
   - 风格感知逻辑：
     - atr_rank > 0.7 (高波动/恐慌): 加大 volatility_20 和 rsi_14 负相关权重 (防守)
     - atr_rank < 0.3 (低波动/阴跌或平稳): 加大 smart_money_ratio 和 momentum_10 权重 (进攻)

5. 新增因子 (New Factors)
   - liquidity_stress: 流动性压力因子，衡量成交额相对变化与价格弹性的关系

【V2.5 问题诊断】
- 因子缺失值比例 100%: volume_entropy_20, momentum_10 等因子在预测时缺失
- ATR Rank 判定不稳定：同一天内模式反复切换
- 领头羊保护参数过松：5% buffer + 5x ATR 导致收益回吐
- 15 天死限不合理：强制退出导致优质标的过早卖出

【解决方案】
1. 因子健壮性：
   - 在 compute_factors 最后一步执行 fill_null
   - 增加 _check_factor_missing 方法，记录缺失值告警
   
2. ATR Rank 稳定化：
   - 使用固定的 250 天窗口
   - 添加 atr_rank_history 平滑处理，使用 EMA 滤波

3. 领头羊保护：
   - LEADER_TOP_PERCENT = 0.03 (Top 3%)
   - LEADER_BUFFER = 0.015 (1.5%)
   - LEADER_ATR_MULTIPLIER = 2.0 (2.0x)
   - 废除 MAX_HOLD_DAYS，改用动态退出逻辑

4. 动态 Alpha:
   - 在 predict_ensemble 中根据 atr_rank 调整因子权重
   - 高波动：volatility_20 权重 x1.5, rsi_14 负向权重 x1.3
   - 低波动：smart_money_ratio 权重 x1.4, momentum_10 权重 x1.3

作者：量化策略团队
版本：Final Strategy V2.6 (Iteration 27)
日期：2026-03-16
"""
...
# ===========================================
# 配置常量 - Iteration 27 更新
# ===========================================

# 【Iteration 27】多模型集成参数
ENSEMBLE_ENABLED = True
RIDGE_WEIGHT = 0.5      # Ridge 基础权重 50%
LGB_WEIGHT = 0.5        # LightGBM 基础权重 50%

# 【Iteration 27】动态权重调整参数 - Inverted Regime Mapping
...
# 【Iteration 27】领头羊保护机制 (Alpha Leader Protection) - 参数收紧
LEADER_PROTECTION_ENABLED = True
LEADER_TOP_PERCENT = 0.03      # Top 3% 标记为领头羊 (从 10% 收紧)
LEADER_BUFFER = 0.015          # 领头羊固定 buffer 1.5% (从 5% 收紧)
LEADER_ATR_MULTIPLIER = 2.0    # 领头羊 ATR 乘数 2.0x (从 5.0x 收紧)

# 【Iteration 27】信号锁定逻辑
PREEMPTIVE_LOCK_ENABLED = True
PREEMPTIVE_LOCK_TOP_PERCENT = 0.10  # 持仓 Top 10% 强制 100% LGB

# 【Iteration 27】止盈逻辑 - 动态持有期
TRAILING_STOP_RELAX_ENABLED = True
TRAILING_STOP_ATR_MULTIPLIER = 3.0  # 统一 ATR 乘数 3.0x
TRAILING_STOP_BUFFER = 0.03         # 统一 buffer 3%

# 【Iteration 27】动态持有期 - 废除 15 天死限
# 改为：如果盈利回撤超过最高点的 30% 且持有超过 10 天则退出
DYNAMIC_HOLD_DAYS_ENABLED = True
MIN_HOLD_DAYS = 10                  # 最小持有天数
PROFIT_PULLBACK_THRESHOLD = 0.30    # 盈利回撤比例阈值
...
# 【Iteration 27】因子缺失值告警阈值
FACTOR_MISSING_THRESHOLD = 0.30  # 30%

# 【Iteration 27】ATR Rank 平滑参数
ATR_RANK_EMA_ALPHA = 0.3  # EMA 平滑系数，避免模式频繁切换
...
class Position:
    ...
    highest_profit_pct: float = 0.0  # 持仓期间最高盈利比例
    is_leader: bool = False        # 是否领头羊
    signal_locked: bool = False    # 是否信号锁定 (100% LGB)
    leader_exit_failed: bool = False  # 领头羊被洗出标记
...
class TradeRecord:
    ...
    is_leader: bool = False         # 是否领头羊
    exited_then_new_high: bool = False  # 被洗出后又创新高
...
class FinalStrategyV26:
    """
    Final Strategy V2.6 - Iteration 27 因子库健壮性修复与 Alpha 增强
    
    核心改进:
        1. 因子库健壮性修复 - fill_null 在最底层执行，消除缺失值报警
        2. 因子缺失值告警 - 超过 30% 缺失时赋 0 权重并记录日志
        3. 领头羊保护参数收紧 - Top 3% 识别，2.0x ATR + 1.5% buffer
        4. 动态持有期 - 废除 15 天死限，改用盈利回撤逻辑
        5. ATR Rank 稳定化 - 250 天滚动窗口 + EMA 平滑
        6. 动态 Alpha 因子集 - 根据 atr_rank 调整因子权重
        7. 新增 liquidity_stress 因子
    """
    ...
    def __init__(
        ...
        # V2.6: ATR Rank 平滑历史
        self.atr_rank_raw_history: List[float] = []
        self.atr_rank_smoothed = 0.5
        ...
        # V2.6 因子缺失值告警记录
        self.factor_missing_alerts: List[Dict[str, Any]] = []
        
        logger.info("FinalStrategyV2.6 initialized")
    ...
    def _compute_atr_rank(self, df: pl.DataFrame, symbol: str, current_atr: float, window: int = 250) -> float:
        ...
        try:
            ...
            if idx < len(ranks):
                atr_rank_raw = float(ranks[idx])
            else:
                atr_rank_raw = 0.5
            
            atr_rank_raw = max(0.0, min(1.0, atr_rank_raw))
            
            # V2.6: 添加 EMA 平滑，避免模式频繁切换
            self.atr_rank_raw_history.append(atr_rank_raw)
            if len(self.atr_rank_raw_history) == 1:
                self.atr_rank_smoothed = atr_rank_raw
            else:
                # EMA 平滑
                self.atr_rank_smoothed = (
                    ATR_RANK_EMA_ALPHA * atr_rank_raw + 
                    (1 - ATR_RANK_EMA_ALPHA) * self.atr_rank_smoothed
                )
            
            logger.debug(f"[ATR Rank] {symbol}: raw={atr_rank_raw:.3f}, smoothed={self.atr_rank_smoothed:.3f}")
            
            return self.atr_rank_smoothed
    ...
    def _update_dynamic_weights(self, atr: float, symbol: str = None, df: pl.DataFrame = None) -> Tuple[float, float]:
        ...
        if df is not None and symbol is not None:
            ...
            atr_rank = self.atr_rank_smoothed
        ...
        if atr_rank <= ATR_RANK_LOW_THRESHOLD:
            ...
            mid_range = ATR_RANK_HIGH_THRESHOLD - ATR_RANK_LOW_THRESHOLD
    ...
    # =========================================================================
    # 【Iteration 27】因子库健壮性修复
    # =========================================================================
    
    def _check_factor_missing(self, df: pl.DataFrame, date: str = None) -> None:
        """
        【V2.6 新增】检查因子缺失值比例，超过阈值时记录告警
        
        【金融逻辑】
        - 如果某个因子缺失值超过 30%，除了赋 0 权重，还要记录日志告警
        - 告警信息包括：哪个标的、哪天缺失、缺失比例
        ...
        """
        exclude_columns = {
            "symbol", "trade_date", "Date", "ts_code",
            "sharpe_label", "future_max_return", "future_volatility",
            "future_return_5d", "label_5d", "predict_score", "filtered_score",
            "raw_score", "score_after_rsi"
        }
        ...
        for col in factor_columns:
            ...
            if null_ratio > FACTOR_MISSING_THRESHOLD:
                # 找出缺失值严重的标的
                missing_symbols = []
                if "symbol" in df.columns:
                    missing_df = df.filter(null_mask)
                    missing_symbols = missing_df["symbol"].unique().to_list()[:5]  # 只显示前 5 个
                ...
                self.factor_missing_alerts.append(alert_info)
                
                logger.warning(
                    f"[因子缺失告警] 因子='{col}', 缺失比例={null_ratio:.1%}, "
                    f"缺失数量={null_count}/{total_rows}, 日期={date}, "
                    f"影响标的={missing_symbols[:3]}"
                )
    
    def _apply_factor_fill_null(self, df: pl.DataFrame) -> pl.DataFrame:
        """
        【V2.6 关键修复】在表达式最底层执行 fill_null
        
        【修复内容】
        - 对所有因子列执行 fill_null，确保预测时没有缺失值
        - 填充策略：forward_fill -> backward_fill -> 0 (中性值)
        ...
        exclude_columns = {
            "symbol", "trade_date", "Date", "ts_code", "industry",
            "sharpe_label", "future_max_return", "future_volatility",
            "future_return_5d", "label_5d"
        }
        ...
        logger.debug(f"[Fill Null] 已填充 {len(factor_columns)} 个因子列")
        return result
    
    # =========================================================================
    # 【Iteration 27】动态 Alpha 因子集 - 风格感知
    # =========================================================================
    
    def _apply_dynamic_alpha_weights(
        self, 
        X_scaled: np.ndarray, 
        ...
        """
        【V2.6 新增】根据市场风格动态调整 Alpha 因子权重
        
        【风格感知逻辑】
        - atr_rank > 0.7 (高波动/恐慌): 
          - 加大 volatility_20 和 rsi_14 的负相关权重 (防守)
        - atr_rank < 0.3 (低波动/阴跌或平稳):
          - 加大 smart_money_ratio 和 momentum_10 的权重 (进攻)
        
        Args:
            X_scaled: 归一化后的特征矩阵
            feature_cols: 特征列名
            atr_rank: 当前 ATR 百分位排名
            
        Returns:
            调整后的预测分数调整量
        """
        adjustments = np.zeros(len(X_scaled))
        
        # 高波动市场 - 防守模式
        if atr_rank > ATR_RANK_HIGH_THRESHOLD:
            logger.debug(f"[动态 Alpha] 高波动模式 (atr_rank={atr_rank:.2f}), 加大防守因子权重")
            
            # volatility_20: 低波动偏好
            if "volatility_20" in feature_cols:
                vol_idx = feature_cols.index("volatility_20")
                # 波动率越低分数越高 (负相关)
                adjustments -= X_scaled[:, vol_idx] * 0.15
            
            # rsi_14: 超卖偏好
            if "rsi_14" in feature_cols:
                rsi_idx = feature_cols.index("rsi_14")
                # RSI 越低 (超卖) 分数越高 (负相关)
                adjustments -= X_scaled[:, rsi_idx] * 0.10
        
        # 低波动市场 - 进攻模式
        elif atr_rank < ATR_RANK_LOW_THRESHOLD:
            logger.debug(f"[动态 Alpha] 低波动模式 (atr_rank={atr_rank:.2f}), 加大进攻因子权重")
            
            # smart_money_ratio: 聪明钱流入偏好
            if "smart_money_ratio" in feature_cols:
                money_idx = feature_cols.index("smart_money_ratio")
                # 聪明钱流入越多分数越高
                adjustments += X_scaled[:, money_idx] * 0.14
            
            # momentum_10: 动量偏好
            if "momentum_10" in feature_cols:
                mom_idx = feature_cols.index("momentum_10")
                # 动量越强分数越高
                adjustments += X_scaled[:, mom_idx] * 0.13
        
        return adjustments
    
    # =========================================================================
    # 【Iteration 27】新增因子 - Liquidity Stress
    # =========================================================================
    
    def compute_liquidity_stress(self, df: pl.DataFrame, lookback: int = 20) -> pl.DataFrame:
        """
        【V2.6 新增】计算流动性压力因子 (Liquidity Stress)
        
        【金融逻辑】
        - 衡量成交额相对变化与价格弹性的关系
        - 流动性压力 = 成交额变化率 / 价格变化率
        - 值越大，表明单位价格变化需要的成交额越多，流动性压力越大
        
        Args:
            df: 输入 DataFrame
            lookback: 滚动窗口 (默认 20 日)
            
        Returns:
            包含 liquidity_stress 列的 DataFrame
        """
        ...
        # 计算成交额变化率
        amount_ma = pl.col("amount").rolling_mean(window_size=lookback)
        amount_change = (pl.col("amount") - amount_ma) / (amount_ma + 1e-6)
        
        # 计算价格变化率 (绝对值)
        price_change = (pl.col("close") / pl.col("close").shift(1) - 1).abs()
        
        # 计算流动性压力
        # 当价格小幅变化但成交额大幅变化时，流动性压力大
        liquidity_stress = amount_change / (price_change + 1e-6)
        
        # 限制在合理范围内
        liquidity_stress_clipped = liquidity_stress.clip(-5.0, 5.0)
        
        result = result.with_columns([
            liquidity_stress_clipped.alias("liquidity_stress"),
            amount_change.alias("amount_change"),
            price_change.alias("price_change_abs"),
        ])
        
        logger.debug(f"[Liquidity Stress] Computed with lookback={lookback}, output rows={len(result)}")
        return result
    ...
    def predict_ensemble(self, df: pl.DataFrame, atr: float = 0.02, symbol: str = None) -> pl.DataFrame:
        ...
        # V2.6: 使用 Inverted Regime Mapping 计算动态权重
        ...
        # V2.6: 动态 Alpha 因子集调整
        alpha_adjustment = self._apply_dynamic_alpha_weights(
            X_scaled_np, ordered_cols, self.current_atr_rank
        )
        
        # 集成加权 + Alpha 调整
        ensemble_pred = ridge_weight * ridge_pred + lgb_weight * lgb_pred + alpha_adjustment
        
        # V2.6: Pre-emptive Lock - 持仓 Top 10% 强制 100% LGB
    ...
    def update_trailing_stop(self, symbol: str, position: Position, 
        ...
        """
        更新并检查追踪止盈 - 【V2.6 动态持有期】
        
        【改进内容】
        - 废除 15 天死限
        - 改为：盈利回撤 > (1.5 * ATR + 0.01) 且持有 > 10 天时退出
        - 领头羊：buffer=1.5%, ATR 乘数=2.0x
        """
        ...
        if high_price > position.highest_price:
            ...
            position.highest_profit_pct = (high_price - position.entry_price) / (position.entry_price + 1e-6)
        
        # 计算当前盈利比例
        profit_pct = (position.highest_price - position.entry_price) / (position.entry_price + 1e-6)
        
        # V2.6: 获取 ATR 乘数和 Buffer (领头羊特殊对待)
        ...
        # V2.6: 动态持有期检查
        # 如果盈利回撤超过最高点的 30% 且持有超过 10 天，则退出
        if DYNAMIC_HOLD_DAYS_ENABLED and position.hold_days >= MIN_HOLD_DAYS:
            current_profit = (current_price - position.entry_price) / (position.entry_price + 1e-6)
            profit_pullback = (position.highest_profit_pct - current_profit) / (position.highest_profit_pct + 1e-6)
            
            if profit_pullback >= PROFIT_PULLBACK_THRESHOLD:
                leader_info = "(领头羊)" if is_leader else ""
                logger.info(f"[动态持有期退出{leader_info}] {symbol}: 持有={position.hold_days}天, "
                           f"最高盈利={position.highest_profit_pct:.1%}, 当前盈利={current_profit:.1%}, "
                           f"回撤={profit_pullback:.1%} >= 阈值={PROFIT_PULLBACK_THRESHOLD:.0%}")
                return True
    ...
    def compute_relative_strength_sector(self, df: pl.DataFrame, period: int = 20) -> pl.DataFrame:
        """
        【V2.6 优化】计算相对强度因子 (Relative Strength vs Sector)
        """
        result = df.clone().with_columns([
            pl.col("close").cast(pl.Float64, strict=False),
        ])
    ...
    def compute_all_enhanced_factors(self, df: pl.DataFrame) -> pl.DataFrame:
        """计算所有增强因子 - V2.6 增加 liquidity_stress"""
        result = df.clone()
        ...
        result = self.compute_liquidity_stress(result, lookback=20)  # V2.6 新增
        return result
    ...
    def run_backtest(self, start_date: str, end_date: str, initial_capital: float = 1000000.0) -> BacktestResult:
        ...
        for date in dates:
            ...
            # V2.6: 因子缺失值检查
            self._check_factor_missing(daily_data, date)
            
            # V2.6: 在预测前执行 fill_null
            daily_data = self._apply_factor_fill_null(daily_data)
    ...
    def run_attribution_analysis(self) -> Dict[str, Any]:
        ...
        trailing_stop_stats = {
            ...
            "washed_out_ratio": washed_out_ratio,
        }
        ...
        return {
            ...
            "factor_missing_alerts": self.factor_missing_alerts[:10],  # 只返回前 10 条
        }
    ...
    def get_weight_trend_analysis(self) -> Dict[str, Any]:
        """
        V2.6: 分析权重变化趋势 (Inverted Regime Mapping)
        """
...
def run_walk_forward_validation(strategy: FinalStrategyV26) -> Dict[str, Any]:
    logger.info("=" * 60)
...
def generate_audit_report(strategy: FinalStrategyV26, walk_forward_result: Dict[str, Any],
                          stress_result: Dict[str, Any], attribution_result: Dict[str, Any],
                          liquidity_stress_result: Optional[Dict[str, Any]] = None) -> str:
    report = []
    report.append("# Iteration 27 全周期审计报告 (V2.6)")
    report.append("")
    report.append(f"**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"**审计版本**: Final Strategy V2.6 (Iteration 27)")
    report.append("")
    ...
    report.append("")
    report.append("Iteration 27 针对因子库健壮性和 Alpha 增强进行了全面升级:")
    report.append("")
    report.append("1. **因子库健壮性修复**: fill_null 在最底层执行，消除缺失值报警")
    report.append("2. **因子缺失值告警**: 超过 30% 缺失时赋 0 权重并记录日志")
    report.append("3. **领头羊保护参数收紧**: Top 3% 识别，2.0x ATR + 1.5% buffer")
    report.append("4. **动态持有期**: 废除 15 天死限，改用盈利回撤逻辑")
    report.append("5. **ATR Rank 稳定化**: 250 天滚动窗口 + EMA 平滑")
    report.append("6. **动态 Alpha 因子集**: 根据 atr_rank 调整因子权重")
    report.append("7. **新增 liquidity_stress 因子**: 衡量流动性压力")
    report.append("")
    ...
    trailing_stats = attribution_result.get("trailing_stop_stats", {})
    leader_analysis = attribution_result.get("leader_analysis", {})
    factor_alerts = attribution_result.get("factor_missing_alerts", [])
    ...
    report.append("### 3.4 因子缺失告警统计")
    report.append("")
    if factor_alerts:
        report.append(f"- **告警次数**: {len(factor_alerts)}")
        report.append("")
        report.append("| 日期 | 因子 | 缺失比例 | 影响标的 |")
        report.append("|------|------|----------|----------|")
        for alert in factor_alerts[:5]:
            symbols = ", ".join(alert.get("affected_symbols", [])[:3])
            report.append(f"| {alert.get('date', 'N/A')} | {alert.get('factor', 'N/A')} | {alert.get('missing_ratio', 0):.1%} | {symbols} |")
    else:
        report.append("✅ **无因子缺失告警**")
    report.append("")
    ...
    report.append("")
    report.append(f"1. **策略有效性**: Iteration 27 在盲测区间 (2024Q1-Q2) 实现 {bt_result.get('total_return', 0):.2%} 收益，")
    if bt_result.get('total_return', 0) > 0.0389:
    ...
    report.append("")
    report.append(f"5. **因子缺失告警**: {len(factor_alerts)} 次")
    report.append("")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Final Strategy V2.6 - Iteration 27")
    logger.info("因子库健壮性修复与 Alpha 增强")
    logger.info("=" * 60)
    
    strategy = FinalStrategyV26(
    ...
    report_path = Path("reports/Iteration27_V26_Audit_Report.md")
    ...
    logger.info(f"因子缺失告警：{len(attribution_result.get('factor_missing_alerts', []))} 次")
    logger.info("=" * 60)
