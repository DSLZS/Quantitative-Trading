"""
Final Strategy V1.6 - Iteration 16: 系统加固、技术债务清理与自适应 Alpha 增强

【核心目标】
1. 技术债务清理 - 统一命名规范、Polars 语法升级、日期上下文修复
2. 核心算法进化：自适应平滑 (Adaptive Smoothing)
3. 极端流动性压力测试 - 流动性折价与滑点增加场景

【Iteration 16 核心改进】

1. 动态 EMA (Adaptive EMA Smoothing):
   - 问题：固定 EMA_ALPHA=0.5 无法适应不同市场环境
   - 解决方案：根据市场波动率 (VIX 或滚动 Std) 动态调整 Alpha
   - 逻辑：高波动时提高 Alpha (增加灵敏度)，低波动时降低 Alpha (增加稳定性)
   - 效果：自适应市场环境，平衡灵敏度与稳定性

2. 行业集中度控制 (Industry Concentration Control):
   - 问题：中性化基础上仍可能过度集中单一行业
   - 解决方案：增加"单行业持仓上限"约束 (默认 30%)
   - 逻辑：选股时检查行业暴露，超过上限则降权或排除
   - 效果：降低行业集中风险，提高组合分散度

3. 极端流动性压力测试 (Extreme Liquidity Stress Test):
   - 场景：模拟 2024 年 Q1 流动性危机
   - 参数：流动性折价 (Liquidity Discount) + 滑点增加 (Slippage Multiplier)
   - 逻辑：成交额萎缩 50% 时，滑点增加 3 倍
   - 效果：评估策略在极端情况下的生存能力

4. 技术债务清理:
   - 统一命名规范：sharpe_label
   - Polars 语法升级：min_periods -> min_samples
   - 日期上下文修复：强制要求 trade_date 列

【因子修复】
- 所有 rolling_* 函数使用 min_samples 参数
- compute_label_5d 强制要求 trade_date 列

作者：量化策略团队
版本：V1.6 (Iteration 16)
日期：2026-03-15
"""
...
# ===========================================
# 配置常量 - Iteration 16 更新
# ===========================================

# 【Iteration 16】动态 EMA 参数
DYNAMIC_EMA_ENABLED = True
EMA_ALPHA_BASE = 0.5       # 基础平滑系数
EMA_ALPHA_MIN = 0.3        # 最小 Alpha (高稳定性)
EMA_ALPHA_MAX = 0.7        # 最大 Alpha (高灵敏度)
EMA_VOLATILITY_WINDOW = 20 # 波动率计算窗口
EMA_VOLATILITY_THRESHOLD_LOW = 0.015   # 低波动阈值
EMA_VOLATILITY_THRESHOLD_HIGH = 0.03   # 高波动阈值

# 【Iteration 16】行业集中度控制参数
INDUSTRY_CONCENTRATION_ENABLED = True
MAX_INDUSTRY_WEIGHT = 0.30  # 单行业上限 30%
INDUSTRY_DIVERSIFICATION_MIN = 5  # 最少行业数量

# 评分 EMA 平滑参数 (保留自 V1.5)
...
# 【Iteration 16】极端流动性压力测试参数
LIQUIDITY_STRESS_TEST_ENABLED = False  # 默认关闭，测试时开启
LIQUIDITY_DISCOUNT = 0.5    # 流动性折价 50%
SLIPPAGE_MULTIPLIER = 3.0   # 滑点增加 3 倍
...
@dataclass
class Position:
    """持仓记录 - V1.6 增加 EMA 平滑评分"""
    symbol: str
    entry_date: str
    entry_price: float
    shares: int
    current_score: float
    smoothed_score: float = 0.0  # EMA 平滑后的评分
    hold_days: int = 0
    stop_loss_price: float = 0.0
    take_profit_price: float = 0.0
    industry: str = ""  # 【新增】行业信息
...
class TradeRecord:
    ...
    exit_reason: str
    hold_days: int
    slippage_cost: float = 0.0  # 【新增】滑点成本
...
class BacktestResult:
    ...
    total_slippage_cost: float = 0.0  # 【新增】总滑点成本
...
class FinalStrategyV16:
    """
    Final Strategy V1.6 - Iteration 16 系统加固与自适应 Alpha 增强
    
    核心改进:
        1. 动态 EMA - 根据市场波动率自适应调整平滑系数
        2. 行业集中度控制 - 单行业上限 30%
        3. 极端流动性压力测试 - 流动性折价与滑点增加
        4. 技术债务清理 - 命名规范、Polars 语法、日期上下文
    """
    ...
    def __init__(
        ...
        stress_test_mode: bool = False,  # 【新增】压力测试模式
    ):
        ...
        logger.info("FinalStrategyV1.6 initialized")
    ...
    # =========================================================================
    # 【Iteration 16】动态 EMA - 基于市场波动率自适应调整
    # =========================================================================
    ...
    def compute_market_volatility(self, df: pl.DataFrame, date: str) -> float:
        ...
        【金融逻辑】
        - 使用全市场股票的收益率标准差作为波动率代理
        - 波动率高时，市场不确定性大，需要更灵敏的响应
        - 波动率低时，市场稳定，可以增加平滑度
        
        Args:
            df: 包含 predict_score_final 的 DataFrame
            date: 交易日期
    ...
    def compute_dynamic_ema_alpha(self, current_volatility: float) -> float:
        ...
        【金融逻辑】
        - 波动率 > 高阈值：Alpha = EMA_ALPHA_MAX (0.7)，快速响应
        - 波动率 < 低阈值：Alpha = EMA_ALPHA_MIN (0.3)，增加稳定性
        - 中间区域：线性插值
    ...
    def compute_ema_score(self, symbol: str, current_score: float, date: str = None) -> float:
        """
        计算 EMA 平滑后的评分 (支持动态 Alpha)
        
        Args:
            symbol: 股票代码
            current_score: 当前预测分
            date: 交易日期 (用于计算动态 Alpha)
    ...
    def check_industry_concentration(self, symbol: str, industry: str) -> bool:
        ...
        【金融逻辑】
        - 计算当前持仓中各行业的权重
        - 如果买入后某行业权重超过 MAX_INDUSTRY_WEIGHT，则拒绝买入
        - 确保组合分散在至少 INDUSTRY_DIVERSIFICATION_MIN 个行业中
    ...
    def update_industry_exposure(self) -> None:
        ...
        # 记录行业暴露
        max_industry_weight = max(self.industry_exposure.values()) if self.industry_exposure else 0
        num_industries = len(self.industry_exposure)
        
        logger.debug(f"[行业暴露] 最大行业权重={max_industry_weight:.1%}, 行业数量={num_industries}")
    ...
    def compute_slippage_cost(
        ...
        Returns:
            滑点成本
        """
        base_slippage_rate = 0.001  # 基础滑点 0.1%
            ...
            if "amount" in daily_data.columns:
                ...
                if not stock_data.is_empty():
                    ...
                    if historical_avg and current_amount < historical_avg * liquidity_discount:
                        ...
                        logger.debug(f"[流动性压力] {symbol}: 成交额 {current_amount/1e6:.1f}M < {historical_avg/1e6:.1f}M * {liquidity_discount}, 滑点 x{slippage_multiplier}")
                    else:
                        slippage_rate = base_slippage_rate
                else:
                    slippage_rate = base_slippage_rate
...
def run_walk_forward_validation(strategy: FinalStrategyV16) -> Dict[str, Any]:
    logger.info("=" * 60)
...
def generate_audit_report(
    ...
    liquidity_stress_result: Optional[Dict[str, Any]] = None,
) -> str:
    report = []
    report.append("# Iteration 16 全周期审计报告")
    report.append("")
    report.append(f"**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"**审计版本**: Final Strategy V1.6 (Iteration 16)")
    report.append("")
    ...
    report.append("")
    report.append("Iteration 16 针对系统加固、技术债务清理与自适应 Alpha 增强进行了全面优化:")
    report.append("")
    report.append("1. **动态 EMA**: 基于市场波动率自适应调整平滑系数 (0.3-0.7)")
    report.append("2. **行业集中度控制**: 单行业上限 30%，降低行业集中风险")
    report.append("3. **极端流动性压力测试**: 流动性折价 50% + 滑点增加 3 倍")
    report.append("4. **技术债务清理**: 统一命名规范、Polars 语法升级、日期上下文修复")
    report.append("")
    ...
    if liquidity_stress_result:
        report.append("## 四、极端流动性压力测试")
        report.append("")
        report.append("### 4.1 测试场景")
        report.append("")
        report.append("| 参数 | 设置 |")
        report.append("|------|------|")
        report.append(f"| 流动性折价 | {LIQUIDITY_DISCOUNT:.0%} |")
        report.append(f"| 滑点倍数 | {SLIPPAGE_MULTIPLIER:.1f}x |")
        report.append("")
        
        report.append("### 4.2 测试结果")
        report.append("")
        report.append("| 指标 | 正常模式 | 压力模式 | 差异 |")
        report.append("|------|----------|----------|------|")
        normal_return = liquidity_stress_result.get('normal_return', 0)
        stress_return = liquidity_stress_result.get('stress_return', 0)
        return_diff = normal_return - stress_return
        report.append(f"| 总收益率 | {normal_return:.2%} | {stress_return:.2%} | {return_diff:.2%} |")
        report.append(f"| 滑点成本 | {liquidity_stress_result.get('normal_slippage', 0):.2f} | {liquidity_stress_result.get('stress_slippage', 0):.2f} | {liquidity_stress_result.get('slippage_diff', 0):.2f} |")
        report.append("")
        
        if return_diff < 0.1:
            report.append("✅ **策略在极端流动性压力下表现稳健**")
        else:
            report.append("⚠️ **策略在极端流动性压力下收益显著下降**")
        report.append("")
    else:
    ...
    report.append("")
    report.append(f"1. **策略有效性**: Iteration 16 在盲测区间 (2024Q1-Q2) 实现 {bt_result.get('total_return', 0):.2%} 收益，")
    ...
    report.append("")
    report.append(f"3. **自适应 Alpha**: 动态 EMA 根据市场波动率自动调整平滑系数，")
    report.append(f"   平衡灵敏度与稳定性")
    report.append("")
    report.append(f"4. **行业分散**: 单行业上限 30% 有效降低了行业集中风险")
    report.append("")
    
    report.append("### 6.2 后续优化方向")
    report.append("")
    report.append("1. 引入更多市场状态识别维度")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Final Strategy V1.6 - Iteration 16")
    logger.info("系统加固、技术债务清理与自适应 Alpha 增强")
    logger.info("=" * 60)
    ...
    report_path = Path("reports/Iteration16_Full_Cycle_Audit_Report.md")
