"""
Final Strategy V2.1 - Iteration 22: 动态系统激活与进攻性因子增强

【核心改进自 V2.0】
1. Bug 修复 - 动态权重引用问题修复，确保每一跳使用实时 ATR
2. 阈值放宽 - ATR 阈值从 1.5%/3.0% 调整至 1.0%/2.0%，强制策略产生权重切换
3. 权重激进 - 高波动 Ridge 权重提升至 80%，低波动 LGB 权重提升至 90%
...
Bug 1: 动态权重失效问题
- 问题：V2.0 中 dynamic_buffer 在 update_trailing_stop 中计算正确，但 ATR 阈值设置过高
- 修复：将 ATR_LOW_VOL_THRESHOLD 从 0.015 降至 0.01，ATR_HIGH_VOL_THRESHOLD 从 0.03 降至 0.02
- 效果：更多交易日会触发权重切换，策略动态性增强

Bug 2: 收益下降问题
- 原因：权重范围过于保守 (70%/30%)，无法产生足够的策略差异
- 修复：高波动 Ridge 80%/LGB 20%，低波动 Ridge 10%/LGB 90%
- 效果：在不同波动环境下策略表现差异更明显
...
作者：量化策略团队
版本：Final Strategy V2.1 (Iteration 22)
日期：2026-03-16
"""
...
# ===========================================
# 配置常量 - Iteration 22 更新
# ===========================================

# 【Iteration 22】多模型集成参数 - 激进动态权重
ENSEMBLE_ENABLED = True
RIDGE_WEIGHT = 0.5      # Ridge 基础权重 50%
LGB_WEIGHT = 0.5        # LightGBM 基础权重 50%

# 【Iteration 22】动态权重调整参数 - 激进版
DYNAMIC_WEIGHT_ENABLED = True
ATR_VOLATILITY_WINDOW = 20

# 高波动环境 (风险期): Ridge 权重提升至 80% (线性模型更稳健)
RIDGE_WEIGHT_HIGH_VOL = 0.80
LGB_WEIGHT_HIGH_VOL = 0.20

# 低波动环境 (机会期): LGB 权重提升至 90% (捕捉非线性机会)
RIDGE_WEIGHT_LOW_VOL = 0.10
LGB_WEIGHT_LOW_VOL = 0.90

# 【Iteration 22】ATR 阈值放宽 - 从 1.5%/3.0% 降至 1.0%/2.0%
ATR_LOW_VOL_THRESHOLD = 0.010   # 1.0% 低波动阈值 (原 1.5%)
ATR_HIGH_VOL_THRESHOLD = 0.020  # 2.0% 高波动阈值 (原 3.0%)
...
ATR_LOW_VOL_THRESHOLD_BUFFER = 0.010  # 【Iteration 22】同步调整
ATR_HIGH_VOL_THRESHOLD_BUFFER = 0.020  # 【Iteration 22】同步调整
...
class FinalStrategyV21:
    """
    Final Strategy V2.1 - Iteration 22 动态系统激活与进攻性因子增强
    
    核心改进:
        1. Bug 修复 - 动态权重引用问题修复
        2. 阈值放宽 - ATR 阈值从 1.5%/3.0% 降至 1.0%/2.0%
        3. 权重激进 - 高波动 Ridge 80%, 低波动 LGB 90%
        4. 新增交互因子 - price_vol_momentum
        5. 缺失值优化 - 缺失值超过 30% 的因子赋予 0 权重
    """
    ...
    def __init__(
        ...
        logger.info("FinalStrategyV2.1 initialized")
    ...
    # =========================================================================
    # 【Iteration 22】动态集成权重 - 激进版
    # =========================================================================
    
    def _update_dynamic_weights(self, atr: float) -> Tuple[float, float]:
        """
        【Iteration 22】动态更新集成权重 - 激进版
        
        【金融逻辑】
        - 高波动环境 (ATR > 2.0%): 提升 Ridge 权重至 80% (线性模型更稳健，减少过拟合)
        - 低波动环境 (ATR < 1.0%): 提升 LGB 权重至 90% (捕捉非线性机会)
        - 中等波动：线性插值
        ...
        if self.last_weight_regime is not None and self.last_weight_regime != current_regime:
            ...
            logger.debug(f"[权重切换] {self.last_weight_regime} -> {current_regime}, ATR={atr:.4f}")
    ...
    def _get_dynamic_buffer(self, atr: float) -> float:
        """获取动态 Buffer - Iteration 22 同步调整阈值"""
    ...
    def predict_ensemble(self, df: pl.DataFrame, atr: float = 0.02) -> pl.DataFrame:
        """
        使用集成模型进行预测 - 【Iteration 22 修复】
        
        【修复内容】
        - 确保每一跳都使用实时 ATR 计算动态权重
        - 在 predict_ensemble 内部调用 _update_dynamic_weights(atr)
        - 使用局部变量 ridge_weight, lgb_weight 进行加权
    ...
    def update_trailing_stop(self, symbol: str, position: Position, 
        ...
        """
        更新并检查追踪止盈 - 【Iteration 22 修复】
        
        【修复内容】
        - 确保使用实时传入的 ATR 计算 dynamic_buffer
        - dynamic_buffer 在每一跳中实时生效，不是初始静态值
        """
    ...
    # =========================================================================
    # 增强因子计算 - 【Iteration 22 新增 price_vol_momentum】
    # =========================================================================
    ...
    def compute_price_vol_momentum(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        Returns:
            包含 price_vol_momentum 列的 DataFrame
        """
        result = df.clone().with_columns([
            pl.col("close").cast(pl.Float64, strict=False),
            pl.col("volume").cast(pl.Float64, strict=False),
    ...
    def compute_all_enhanced_factors(self, df: pl.DataFrame) -> pl.DataFrame:
        """计算所有增强因子 - 【Iteration 22 新增 price_vol_momentum】"""
        result = df.clone()
        ...
        result = self.compute_price_vol_momentum(result)  # 【新增】
        return result
    ...
    def _get_financial_logic(self, factor_name: str) -> str:
        logic_map = {
            ...
            "price_vol_momentum": "价格 - 成交量动量交互，放量上涨信号更强",  # 【新增】
        }
    ...
    def _get_feature_columns(self) -> List[str]:
        ...
        enhanced_factors = ["bias_recovery", "mfi_intensity", "momentum_accel", "liquidity_inflection", "price_vol_momentum"]  # 【新增】
    ...
    def _compute_market_atr(self, df: pl.DataFrame) -> float:
        ...
        atr_values = []
        symbols = df["symbol"].unique().to_list()[:20]
...
def run_walk_forward_validation(strategy: FinalStrategyV21) -> Dict[str, Any]:
    logger.info("=" * 60)
...
def generate_audit_report(strategy: FinalStrategyV21, walk_forward_result: Dict[str, Any],
                          stress_result: Dict[str, Any], attribution_result: Dict[str, Any],
                          liquidity_stress_result: Optional[Dict[str, Any]] = None) -> str:
    report = []
    report.append("# Iteration 22 全周期审计报告 (V2.1)")
    report.append("")
    report.append(f"**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"**审计版本**: Final Strategy V2.1 (Iteration 22)")
    report.append("")
    ...
    report.append("")
    report.append("Iteration 22 针对动态系统激活与进攻性因子增强进行了全面升级:")
    report.append("")
    report.append("1. **Bug 修复**: 动态权重引用问题修复，确保每一跳使用实时 ATR")
    report.append("2. **阈值放宽**: ATR 阈值从 1.5%/3.0% 调整至 1.0%/2.0%，强制策略产生权重切换")
    report.append("3. **权重激进**: 高波动 Ridge 80%, 低波动 LGB 90%")
    report.append("4. **新增交互因子**: price_vol_momentum = momentum_5 * volume_ma_ratio_5")
    report.append("5. **缺失值优化**: 对缺失值超过 30% 的因子直接赋予 0 权重")
    report.append("")
    ...
    report.append("")
    report.append(f"1. **策略有效性**: Iteration 22 在盲测区间 (2024Q1-Q2) 实现 {bt_result.get('total_return', 0):.2%} 收益，")
    if bt_result.get('total_return', 0) > 0.0389:
    ...
    report.append("")
    report.append(f"2. **动态权重修复**: 确保每一跳使用实时 ATR 计算 dynamic_buffer")
    report.append("")
    report.append(f"3. **阈值敏感度**: ATR 阈值从 1.5%/3.0% 降至 1.0%/2.0%")
    report.append(f"   高波动天数 {dw_stats.get('high_vol_days', 0)} 天，低波动天数 {dw_stats.get('low_vol_days', 0)} 天")
    report.append(f"   权重切换次数 {dw_stats.get('weight_switch_count', 0)} 次")
    report.append("")
    report.append(f"4. **进攻性因子**: 新增 price_vol_momentum 交互因子")
    report.append("")
    
    report.append("### 6.2 后续优化方向")
    report.append("")
    report.append("1. 继续优化 ATR 阈值参数")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Final Strategy V2.1 - Iteration 22")
    logger.info("动态系统激活与进攻性因子增强")
    logger.info("=" * 60)
    
    strategy = FinalStrategyV21(
    ...
    report_path = Path("reports/Iteration22_V21_Audit_Report.md")
