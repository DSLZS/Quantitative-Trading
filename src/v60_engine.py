"""
V60 Engine Module - 回测引擎与真正的逻辑自进化

【核心功能】
1. V60BacktestEngine - 完整的回测执行引擎
2. MasterLoop - 真正的逻辑自进化（收益率<15% 时动态调整）
3. 逻辑突变记录 - 详细记录每次逻辑变化
...
from v60_core import (
    V60Position, V60Trade, V60TradeAudit, V60WashSaleRecord,
    V60BlacklistRecord, V60MarketRegime, V60DrawdownState,
    V60WeeklyTradeCounter, V60IterationResult, V60LogicEvolutionRecord,
    V60FactorEngine, V60RiskManager, V60IndustryLoader,
    v60_industry_filter,
    V60_INITIAL_CAPITAL, V60_MAX_POSITIONS,
    V60_WEEKLY_TRADE_LIMIT, V60_GLOBAL_TRADE_LIMIT,
    V60_ENTRY_TOP_N, V60_INDUSTRY_TOP_N,
    V60_COMMISSION_RATE, V60_MIN_COMMISSION,
    V60_SLIPPAGE_BUY, V60_SLIPPAGE_SELL,
    V60_STAMP_DUTY, V60_TRANSFER_FEE,
    V60_WASH_SALE_WINDOW, V60_FRICTION_COST,
    V60_MAX_ITERATION_ROUNDS, V60_RETURN_TARGET,
    V60_MDD_TARGET, V60_PROFIT_LOSS_RATIO_TARGET,
    V60_MOMENTUM_WEIGHT, V60_R2_WEIGHT, V60_TREND_WEIGHT,
    V60_RS_TOP_PERCENTILE, V60_VOLUME_BREAKOUT_MULT,
    V60_HARD_STOP_LOSS_ATR_MULT, V60_BREAKEVEN_PROFIT_THRESHOLD,
    V60_TRAILING_PROFIT_TRIGGER, V60_MAX_SINGLE_POSITION_PCT,
    V60_MA20_BREAKOUT, V60_DYNAMIC_EVOLUTION_ENABLED,
    V60_MIN_SELECTION_PERCENTILE, V60_MAX_SELECTION_PERCENTILE,
    V60_MIN_TREND_PERIOD, V60_MAX_TREND_PERIOD,
)


@dataclass
class V60AccountState:
    """V60 账户状态"""
    trade_date: str = ""
    total_equity: float = V60_INITIAL_CAPITAL
    available_cash: float = V60_INITIAL_CAPITAL
    position_market_value: float = 0.0
    daily_pnl: float = 0.0
    daily_pnl_ratio: float = 0.0
    total_pnl: float = 0.0
    total_pnl_ratio: float = 0.0
    peak_equity: float = V60_INITIAL_CAPITAL
...
class V60BacktestResult:
    ...
    winning_trades: int = 0
    losing_trades: int = 0
    final_equity: float = V60_INITIAL_CAPITAL
    initial_equity: float = V60_INITIAL_CAPITAL
    total_commission: float = 0.0
    total_slippage: float = 0.0
    total_stamp_duty: float = 0.0
    ...
    trades: List[V60Trade] = field(default_factory=list)
    positions: List[Dict] = field(default_factory=list)
    audits: List[V60TradeAudit] = field(default_factory=list)
...
class V60BacktestEngine:
    """
    V60 回测引擎 - 完整执行引擎
    
    【核心功能】
    1. 全样本数据加载 - 严禁 limit 20
    2. ATR 趋势跟踪 2.0（MA20>MA60 且 Close>MA120）
    3. 动态止损（废除 8% 固定止损，使用 ATR 动态止损）
    4. 利润生长空间（浮盈<15% 不启动止盈/保本）
    5. 成交价审计（min(Trigger, Next_Open)）
    """
    
    def __init__(self, initial_capital: float = V60_INITIAL_CAPITAL):
        self.initial_capital = initial_capital
        self.account = V60AccountState(total_equity=initial_capital, available_cash=initial_capital, peak_equity=initial_capital)
        self.positions: Dict[str, V60Position] = {}
        self.trades: List[V60Trade] = []
        self.trade_audits: List[V60TradeAudit] = []
        self.wash_sale_records: List[V60WashSaleRecord] = []
        self.blacklist: Dict[str, V60BlacklistRecord] = {}
        self.weekly_counters: Dict[int, V60WeeklyTradeCounter] = {}
        self.equity_curve: List[float] = []
        self.drawdown_curve: List[float] = []
        self.trade_dates: List[str] = []
        self.price_audit_violations = 0
        
        self.factor_engine = V60FactorEngine()
        self.risk_manager = V60RiskManager()
    ...
    def run_backtest(self, factor_data: pl.DataFrame, price_data: pl.DataFrame,
        ...
        """运行完整回测"""
        try:
            logger.info("V60 Backtest Engine Starting...")
            ...
            if unique_stocks < 100:
                logger.error(f"V60 FATAL: Data fraud detected! Only {unique_stocks} stocks loaded.")
                logger.error("V60 RULE: Must load full market data (>500 stocks)")
    ...
    def _update_weekly_counter(self, trade_date: str):
        ...
        try:
            ...
            if week_key not in self.weekly_counters:
                self.weekly_counters[week_key] = V60WeeklyTradeCounter(
                    week_number=week,
                    year=year,
                    trade_count=0
                )
    ...
    def _execute_exit(self, trade_date: str, symbol: str, reason: str):
        ...
        sell_price = next_open * (1 - V60_SLIPPAGE_SELL)
        
        shares = position.shares
        sell_amount = sell_price * shares
        
        commission = max(sell_amount * V60_COMMISSION_RATE, V60_MIN_COMMISSION)
        stamp_duty = sell_amount * V60_STAMP_DUTY
        transfer_fee = sell_amount * V60_TRANSFER_FEE
        ...
        trade = V60Trade(
            ...
            slippage_applied=V60_SLIPPAGE_SELL,
    ...
    def _audit_execution_price(self, trade: V60Trade) -> bool:
        """
        成交价审计 - V60 终极禁令
        
        【规则】
        每笔交易必须符合 min(Trigger, Next_Open)
        严禁出现"卖价 > 触发价"的奇迹
        """
        trigger_price = trade.trigger_price
        ...
        if trade.side == 'SELL':
            if execution_price > min_price * 1.01:
                ...
                logger.warning(f"  Trigger: {trigger_price:.2f}, Next Open: {next_open:.2f}")
                logger.warning(f"  Execution: {execution_price:.2f}, Min: {min_price:.2f}")
                return False
        else:
    ...
    def _process_entries(self, trade_date: str, current_df: pl.DataFrame,
        ...
        if len(self.positions) >= V60_MAX_POSITIONS:
            return
        
        if self.global_trade_count >= V60_GLOBAL_TRADE_LIMIT:
            return
        
        weekly_counter = self.weekly_counters.get(self.current_year * 100 + self.current_week)
        if weekly_counter and weekly_counter.trade_count >= V60_WEEKLY_TRADE_LIMIT:
            return
        
        # 行业过滤
        filtered_df, filter_stats = v60_industry_filter(
            factor_df, industry_data, self.industry_loader,
            trade_date, V60_INDUSTRY_TOP_N, index_data
        )
        
        # 获取候选股票
        candidates = filtered_df.sort('composite_score', descending=True).head(V60_ENTRY_TOP_N)
        ...
        for row in candidates.iter_rows(named=True):
            ...
            if V60_MA20_BREAKOUT and not trend_confirmed:
                continue
            ...
            buy_price = current_price * (1 + V60_SLIPPAGE_BUY)
            ...
            max_position_value = self.account.available_cash * V60_MAX_SINGLE_POSITION_PCT
            ...
            commission = max(buy_amount * V60_COMMISSION_RATE, V60_MIN_COMMISSION)
            transfer_fee = buy_amount * V60_TRANSFER_FEE
            ...
            position = V60Position(
                ...
                hard_stop_price=buy_price - (V60_HARD_STOP_LOSS_ATR_MULT * current_atr),
            ...
            trade = V60Trade(
                ...
                amount=buy_amount,
                commission=commission,
                slippage=V60_SLIPPAGE_BUY,
                stamp_duty=0,
                transfer_fee=transfer_fee,
                total_cost=total_cost,
                reason=f"Entry Signal (Score={composite_score:.4f}, Trend={trend_confirmed})",
                execution_price=buy_price,
                signal_date=trade_date,
                trigger_price=current_price,
                next_open_price=current_price,
                min_trigger_open=min(current_price, current_price),
                slippage_applied=V60_SLIPPAGE_BUY,
                price_audit_passed=True
            )
        ...
        for record in self.wash_sale_records:
            if record.symbol == symbol:
                try:
                    ...
                    if 0 < days_diff <= V60_WASH_SALE_WINDOW:
                        return True
                except Exception:
                    pass
        return False
    ...
    def _add_to_blacklist(self, symbol: str, trade_date: str, reason: str):
        ...
        try:
            current = datetime.strptime(trade_date, "%Y-%m-%d")
            expiry = current + timedelta(days=V60_WASH_SALE_WINDOW)
            
            self.blacklist[symbol] = V60BlacklistRecord(
                symbol=symbol,
                stop_date=trade_date,
                stop_reason=reason,
                blacklist_expiry_day=int(expiry.strftime("%Y%m%d")),
                days_remaining=V60_WASH_SALE_WINDOW
            )
        except Exception:
            pass
    ...
    def _check_compliance(self, trade_date: str):
        """检查合规性"""
        if self.global_trade_count > V60_GLOBAL_TRADE_LIMIT:
            logger.warning(f"COMPLIANCE VIOLATION: Global trade limit exceeded ({self.global_trade_count} > {V60_GLOBAL_TRADE_LIMIT})")
        
        weekly_counter = self.weekly_counters.get(self.current_year * 100 + self.current_week)
        if weekly_counter and weekly_counter.trade_count > V60_WEEKLY_TRADE_LIMIT:
            logger.warning(f"COMPLIANCE VIOLATION: Weekly trade limit exceeded ({weekly_counter.trade_count} > {V60_WEEKLY_TRADE_LIMIT})")
    ...
    def _generate_result(self) -> Dict[str, Any]:
        ...
        meets_target = (
            total_return >= V60_RETURN_TARGET and
            self.account.max_drawdown <= V60_MDD_TARGET and
            profit_loss_ratio >= V60_PROFIT_LOSS_RATIO_TARGET
        )
        
        target_analysis = []
        if total_return < V60_RETURN_TARGET:
            target_analysis.append(f"Return {total_return:.2%} < {V60_RETURN_TARGET:.1%}")
        if self.account.max_drawdown > V60_MDD_TARGET:
            target_analysis.append(f"MDD {self.account.max_drawdown:.2%} > {V60_MDD_TARGET:.1%}")
        if profit_loss_ratio < V60_PROFIT_LOSS_RATIO_TARGET:
            target_analysis.append(f"P/L {profit_loss_ratio:.2f} < {V60_PROFIT_LOSS_RATIO_TARGET:.1f}")
...
class MasterLoop:
    """
    MasterLoop - V60 真正的逻辑自进化
    
    【核心逻辑】
    1. 摧毁 max_iterations = 5 的锁死 - 最多 50 轮迭代
    2. 收益率<15% 时动态调整选股分位数和趋势确认周期
    3. 逻辑差异化审计：展示不同逻辑下的选股差异
    
    【进化策略】
    - 如果 Profit_Loss_Ratio < 2.5 或 Total_Return < 15%：
      - 强制动作 A：更换选股因子（动量/R2/趋势权重调整）
      - 强制动作 B：调整选股分位数（Percentile）
      - 强制动作 C：调整趋势确认周期
    """
    
    def __init__(self, max_iterations: int = V60_MAX_ITERATION_ROUNDS):
        self.max_iterations = max_iterations
        self.iteration_results: List[V60IterationResult] = []
        self.logic_evolution_records: List[V60LogicEvolutionRecord] = []
        self.current_logic_path = 0
        
        # 逻辑路径定义
        self.logic_paths = [
            'momentum_r2_trend',      # 动量+R2+ 趋势
            'rsrs_momentum',           # RSRS+ 动量
            'volatility_quality',      # 波动率 + 质量
        ...
        self.current_parameters = {
            'momentum_weight': V60_MOMENTUM_WEIGHT,
            'r2_weight': V60_R2_WEIGHT,
            'trend_weight': V60_TREND_WEIGHT,
            'entry_top_n': V60_ENTRY_TOP_N,
            'rs_top_percentile': V60_RS_TOP_PERCENTILE,
            'volume_breakout_mult': V60_VOLUME_BREAKOUT_MULT,
            'hard_stop_atr_mult': V60_HARD_STOP_LOSS_ATR_MULT,
            'breakeven_threshold': V60_BREAKEVEN_PROFIT_THRESHOLD,
            'trailing_profit_trigger': V60_TRAILING_PROFIT_TRIGGER,
            'selection_percentile': 0.15,  # 选股分位数
            'trend_period': 20,  # 趋势确认周期
        }
    ...
    def run_iteration(self, factor_data: pl.DataFrame, price_data: pl.DataFrame,
        ...
        meets_target = (
            total_return >= V60_RETURN_TARGET and
            profit_loss_ratio >= V60_PROFIT_LOSS_RATIO_TARGET and
            max_drawdown <= V60_MDD_TARGET
        )
        
        iteration_result = V60IterationResult(
            iteration=iteration,
        ...
        # 记录逻辑进化
        if self.current_logic_path != iteration_result.iteration or previous_params != self.current_parameters:
            evolution_record = V60LogicEvolutionRecord(
                iteration=iteration,
    ...
    def _adapt_strategy_on_error(self, error_msg: str):
        ...
        self.current_parameters['selection_percentile'] = min(
            self.current_parameters.get('selection_percentile', 0.15) + 0.05, 0.30
        )
    ...
    def _adapt_strategy(self, total_return: float, profit_loss_ratio: float, max_drawdown: float):
        ...
        if profit_loss_ratio < V60_PROFIT_LOSS_RATIO_TARGET:
            ...
            # 调整选股分位数
            current_percentile = self.current_parameters.get('selection_percentile', 0.15)
            if current_percentile < V60_MAX_SELECTION_PERCENTILE:
                self.current_parameters['selection_percentile'] = min(
                    current_percentile + 0.05, V60_MAX_SELECTION_PERCENTILE
                )
        ...
        if total_return < V60_RETURN_TARGET:
            ...
            if current_trend_period > V60_MIN_TREND_PERIOD:
                self.current_parameters['trend_period'] = max(
                    current_trend_period - 5, V60_MIN_TREND_PERIOD
                )
            
            # 调整止盈阈值（降低门槛，让更多利润生长）
            if self.current_parameters.get('trailing_profit_trigger', 0.20) > 0.15:
                self.current_parameters['trailing_profit_trigger'] = max(
                    self.current_parameters['trailing_profit_trigger'] - 0.02, 0.15
                )
            
            self.current_logic_path += 1
        
        # 3. 回撤超标
        if max_drawdown > V60_MDD_TARGET:
            evolution_step = "回撤超标 -> 提高止损倍数 + 降低仓位"
            logger.info(f"风控调整：{evolution_step}")
            
            self.current_parameters['hard_stop_atr_mult'] = min(
                self.current_parameters.get('hard_stop_atr_mult', 3.0) + 0.5, 4.0
            )
            self.current_parameters['selection_percentile'] = max(
                self.current_parameters.get('selection_percentile', 0.15) - 0.02, V60_MIN_SELECTION_PERCENTILE
            )
    ...
    def _get_evolution_reason(self, total_return: float, profit_loss_ratio: float, max_drawdown: float) -> str:
        ...
        reasons = []
        if profit_loss_ratio < V60_PROFIT_LOSS_RATIO_TARGET:
            reasons.append(f"P/L {profit_loss_ratio:.2f} < {V60_PROFIT_LOSS_RATIO_TARGET}")
        if total_return < V60_RETURN_TARGET:
            reasons.append(f"Return {total_return:.2%} < {V60_RETURN_TARGET}")
        if max_drawdown > V60_MDD_TARGET:
            reasons.append(f"MDD {max_drawdown:.2%} > {V60_MDD_TARGET}")
    ...
    def get_best_result(self) -> Optional[V60IterationResult]:
        """获取最佳迭代结果"""
