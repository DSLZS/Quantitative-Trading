"""
V94 Engine - 回归 V90 基准与成交量加权动量增强
...
【V94 硬性指标】
- 指标 A (Predictive Power): T+1 Rank IC >= 0.045
- 指标 B (Risk Control): 最大回撤 < 15%
...
class V94EngineConfig:
    ...
    def __init__(
        ...
        initial_capital: float = V94_INITIAL_CAPITAL,
        max_positions: int = V94_MAX_POSITIONS,
        warmup_period: int = V94_WARMUP_PERIOD,
        commission_rate: float = V94_COMMISSION_RATE,
        min_commission: float = V94_MIN_COMMISSION,
        stamp_duty: float = V94_STAMP_DUTY,
        transfer_fee: float = V94_TRANSFER_FEE,
        oos_years: List[str] = None,
        min_score_threshold: float = V94_MIN_SCORE_THRESHOLD,
        min_single_weight: float = V94_MIN_SINGLE_WEIGHT,
        max_single_weight: float = V94_MAX_SINGLE_WEIGHT,
        # V94 配置
        enable_style_neutralization: bool = True,
        enable_liquidity_shock: bool = True,
        enable_dynamic_rebalance: bool = True,
        enable_vwap_momentum: bool = True,  # V94 新增
    ):
...
class V94Engine:
    ...
    def __init__(self, config: V94EngineConfig = None, db=None):
        self.config = config or V94EngineConfig()
        
        if db is None and DB_AVAILABLE:
            try:
                self.db = get_db()
            except Exception as e:
                logger.error(f"V94: 数据库连接失败 - {e}")
                self.db = None
        else:
            self.db = db
        
        self.data_manager = V94DataManager(db=self.db, config={
            'warmup_period': self.config.warmup_period,
        })
        self.single_factor_auditor = V94SingleFactorICAuditor(db=self.db, config={
            'ic_threshold': V94_VWAP_IC_THRESHOLD,
        })
        self.vwap_momentum = V94VWAPMomentumEngine() if self.config.enable_vwap_momentum else None
        self.alpha_fusion = V94AlphaFusion(db=self.db, config={
            'fusion_lags': V94_HALF_LIFE_LAGS,
        })
        self.alpha_weight = V94AlphaWeightEngine(config={
        ...
        self.ic_audit = V94ICAudit(db=self.db)
        self.turnover_tracker = V94TurnoverTracker()
        self.portfolio_tracker = V94PortfolioTracker(
            initial_capital=self.config.initial_capital
        )
        self.style_neutralization = V94StyleNeutralizationEngine() if self.config.enable_style_neutralization else None
        self.liquidity_shock = V94LiquidityShockEngine() if self.config.enable_liquidity_shock else None
        self.dynamic_rebalance = V94DynamicRebalanceEngine() if self.config.enable_dynamic_rebalance else None
        ...
        logger.info("=" * 70)
        logger.info("V94 Engine 初始化完成")
        logger.info("=" * 70)
        logger.info(f"V94: 初始资金={self.config.initial_capital:,.2f} (严禁修改)")
        logger.info(f"V94: VWAP Momentum={'启用' if self.config.enable_vwap_momentum else '禁用'}")
        logger.info(f"V94: VWAP IC 门槛={V94_VWAP_IC_THRESHOLD:.4f}")
        logger.info(f"V94: 风格中性化={'启用' if self.config.enable_style_neutralization else '禁用'}")
        logger.info(f"V94: 流动性冲击={'启用' if self.config.enable_liquidity_shock else '禁用'}")
        logger.info(f"V94: 动态调仓={'启用' if self.config.enable_dynamic_rebalance else '禁用'}")
        logger.info("=" * 70)
    
    def run_backtest(self) -> Dict[str, Any]:
        """运行回测"""
        logger.info("=" * 70)
        logger.info("V94 回归基准与 VWAP Momentum 增强回测引擎启动")
        logger.info("=" * 70)
        
        if self.db is None:
            logger.error("V94: 数据库连接未初始化")
            return self._empty_result()
        
        try:
            # 1. 数据完整性检查
            logger.info("V94: [1/8] 开始数据完整性检查...")
            data_integrity_results = self._check_data_integrity()
            
            # 2. 加载数据
            logger.info("V94: [2/8] 开始加载数据...")
            df = self._load_data()
            
            if df.is_empty():
                logger.error("V94: 未加载到任何数据")
                return self._empty_result()
            ...
            for year in self.config.oos_years:
                ...
                if not df_year.is_empty():
                    ...
                    logger.info(f"V94: {year}年 - 股票数={stock_count}, 交易天数={trading_days}, 总行数={total_rows:,}")
            
            # 3. 计算 V90 因子信号
            logger.info("V94: [3/8] 开始计算 V90 因子信号...")
            df_with_signals = self._compute_v90_signals(df)
            
            # 4. 计算 VWAP Momentum 信号并审计
            if self.config.enable_vwap_momentum:
                logger.info("V94: [4/8] 开始计算 VWAP Momentum 信号...")
                df_with_vwap = self._compute_vwap_momentum(df_with_signals)
                
                # 单因子 IC 审计
                logger.info("V94: 开始单因子 IC 审计...")
                vwap_ic = self.single_factor_auditor.audit_single_factor(
                    df_with_vwap, 'vwap_momentum_score', 'VWAP_Momentum'
                )
                
                # 如果 IC 低于门槛，不计入复合得分
                if not vwap_ic.passed_threshold:
                    logger.warning(f"V94: VWAP Momentum IC ({vwap_ic.ic_t1:.4f}) < 门槛 ({V94_VWAP_IC_THRESHOLD:.4f})，将不计入复合得分")
                    df_with_signals = df_with_vwap.with_columns([
                        pl.lit(0.0).alias('vwap_momentum_score')
                    ])
                else:
                    df_with_signals = df_with_vwap
            else:
                logger.info("V94: [4/8] 跳过 VWAP Momentum 计算")
            
            # 5. 风格中性化
            if self.config.enable_style_neutralization:
                logger.info("V94: [5/8] 开始风格中性化...")
                ...
                logger.info("V94: [5/8] 跳过风格中性化")
            
            # 6. 流动性冲击调整
            if self.config.enable_liquidity_shock:
                logger.info("V94: [6/8] 开始流动性冲击调整...")
                ...
                logger.info("V94: [6/8] 跳过流动性冲击调整")
            
            # 7. 半衰期融合
            logger.info("V94: [7/8] 开始半衰期融合...")
            df_with_fusion = self.alpha_fusion.compute_fusion_signal(
                df_with_signals, signal_col='composite_score'
            )
            
            # 8. Alpha 权重与 IC 审计
            logger.info("V94: [8/8] 开始 Alpha 权重计算与 IC 审计...")
            ...
            logger.info(f"V94: T+1 Rank IC = {ic_audit_results['ic_t1']:.4f} (目标 > {V94_T1_IC_TARGET})")
            logger.info(f"V94: T+2 Rank IC = {ic_audit_results['ic_t2']:.4f}")
            logger.info(f"V94: T+3 Rank IC = {ic_audit_results['ic_t3']:.4f}")
            
            # 执行回测交易
            logger.info("V94: 开始执行回测交易...")
            trade_results = self._execute_backtest(df_with_weights)
            
            # 生成报告
            logger.info("V94: 生成审计报告...")
            ...
            logger.info("=" * 70)
            logger.info("V94 回测完成")
            logger.info("=" * 70)
            
            return result
            
        except Exception as e:
            logger.error(f"V94 回测失败 - {e}")
    ...
    def _check_data_integrity(self) -> Dict[str, Dict[str, Any]]:
        ...
        for year in self.config.oos_years:
            ...
            if passed:
                logger.info(f"V94: {year}年数据检查通过 - {message}")
            else:
                logger.warning(f"V94: {year}年数据检查失败 - {message}")
        return results
    ...
    def _load_data(self) -> pl.DataFrame:
        ...
        for year in self.config.oos_years:
            ...
            try:
                ...
                if not df.is_empty():
                    all_dfs.append(df)
                    logger.info(f"V94: {year}年数据加载成功，行数={df.height:,}")
            except Exception as e:
                logger.warning(f"V94: 加载 {year}年数据失败 - {e}")
        
        if not all_dfs:
            return pl.DataFrame()
        
        combined_df = pl.concat(all_dfs)
        logger.info(f"V94: 总数据行数={combined_df.height:,}")
    ...
    def _compute_composite_score(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        result = result.with_columns([
            (V94_RESIDUAL_WEIGHT * pl.col('refined_residual_score') + 
             V94_FLOW_WEIGHT * pl.col('smart_flow_score') +
             V94_INTERACTION_WEIGHT * pl.col('vol_price_interaction_score')).alias('composite_score')
        ])
    ...
    def _compute_vwap_momentum(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        ic_result = self.single_factor_auditor.audit_single_factor(
            result, 'vwap_momentum_score', 'VWAP_Momentum'
        )
        
        # 如果 IC 低于门槛，将分数置为 0
        if not ic_result.passed_threshold:
            logger.warning(f"V94: VWAP Momentum IC ({ic_result.ic_t1:.4f}) < 门槛，将不计入复合得分")
            result = result.with_columns([
                pl.lit(0.0).alias('vwap_momentum_score')
            ])
    ...
    def _execute_backtest(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        warmup_cutoff = unique_dates[:min(V94_WARMUP_PERIOD, len(unique_dates))]
        trade_dates = [d for d in unique_dates if d not in warmup_cutoff]
        
        logger.info(f"V94: 热身期 {len(warmup_cutoff)} 天，交易期 {len(trade_dates)} 天")
        ...
        for i, trade_date in enumerate(trade_dates):
            ...
            if is_rebalance_day:
                ...
                logger.debug(f"V94: {trade_date} 调仓")
                
                # 放宽筛选条件：只要 alpha_weight 有效即可
            ...
            max_daily_turnover_value = self.portfolio_value * V94_DAILY_TURNOVER_MAX
            ...
            if (i + 1) % 50 == 0:
                logger.info(f"V94: 处理 {i + 1}/{len(trade_dates)} 天，组合价值={self.portfolio_value:,.2f}")
        ...
        logger.info(f"V94: 回测完成 - 总收益={total_return:.2%}, 年化换手={turnover_summary['annualized_turnover']:.2%}")
        logger.info(f"V94: 调仓次数={len(self.rebalance_dates)}, 最大回撤={max_drawdown:.2%}")
    ...
    def _should_rebalance_today(self, trade_date: str, df: pl.DataFrame) -> bool:
        ...
        if not self.config.enable_dynamic_rebalance:
            ...
            if days_diff >= V94_MIN_REBALANCE_INTERVAL:
                return True
    ...
    def _calculate_target_positions(self, valid_stocks: pl.DataFrame, 
        ...
        if total_weight > EPSILON:
            effective_limit = max(0.90, position_limit)
    ...
    def _generate_audit_report(self, data_integrity: Dict, ic_audit: Dict,
        ...
        lines = []
        lines.append("=" * 70)
        lines.append("《V94 回归基准与 VWAP Momentum 增强审计报告》")
        lines.append("=" * 70)
        lines.append("")
        ...
        lines.append("   " + "-" * 50)
        lines.append(f"   T+1 Rank IC: {ic_audit.get('ic_t1', 0.0):.4f} (目标 > {V94_T1_IC_TARGET})")
        ...
        lines.append("5. V94 硬性指标验证")
        lines.append("   " + "-" * 50)
        
        ic_t1 = ic_audit.get('ic_t1', 0.0)
        metric_a_pass = ic_t1 >= V94_T1_IC_TARGET
        ...
        metric_b_pass = max_dd <= 0.15
        lines.append(f"   指标 B (最大回撤 <= 15%): {'✓' if metric_b_pass else '✗'}")
        lines.append(f"     - 最大回撤：{max_dd:.2%}")
        lines.append("")
        
        # 检查三年度 IC 是否全部为正
        all_years_positive = all(v > 0 for v in ic_by_year.values()) if ic_by_year else False
        metric_c_pass = all_years_positive
        lines.append(f"   指标 C (三年度 IC 全部为正): {'✓' if metric_c_pass else '✗'}")
        if ic_by_year:
            for year, ic_val in ic_by_year.items():
                lines.append(f"     - {year}年 IC: {ic_val:.4f}")
        lines.append("")
        
        turnover = trade_results.get('annualized_turnover', 0.0)
        daily_ok = trade_results.get('daily_turnover_ok', False)
        metric_d_pass = (V94_TURNOVER_MIN <= turnover <= V94_TURNOVER_MAX) and daily_ok
        lines.append(f"   指标 D (年化换手 300%-500%, 单日<10%): {'✓' if metric_d_pass else '✗'}")
        lines.append(f"     - 年化换手：{turnover:.2%}")
        lines.append(f"     - 单日换手达标：{'是' if daily_ok else '否'}")
        lines.append("")
        
        lines.append("6. V94 增强特性")
        lines.append("   " + "-" * 50)
...
def run_v94_backtest(config: V94EngineConfig = None) -> Dict[str, Any]:
    """运行 V94 回测"""
    engine = V94Engine(config=config)
    return engine.run_backtest()


def print_v94_report(result: Dict[str, Any]) -> None:
    """打印 V94 报告"""
    logger.info("=" * 70)
    logger.info("V94 最终报告")
    logger.info("=" * 70)
...
if __name__ == "__main__":
    ...
    result = run_v94_backtest(config)
    print_v94_report(result)
    
    output_path = "reports/v94_backtest_result.json"
    ...
    logger.info(f"V94: 结果已保存至 {output_path}")