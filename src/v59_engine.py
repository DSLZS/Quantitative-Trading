"""
V59 Engine Module - 回测引擎与 MasterLoop 迭代协议

【核心功能】
1. V59BacktestEngine - 完整的回测执行引擎
2. MasterLoop - 不达标强制更换逻辑的迭代协议
3. 成交价审计 - min(Trigger, Next_Open) 验证
...
from v59_core import (
    V59Position, V59Trade, V59TradeAudit, V59WashSaleRecord,
    V59BlacklistRecord, V59MarketRegime, V59DrawdownState,
    V59WeeklyTradeCounter, V59IterationResult,
    V59FactorEngine, V59RiskManager, V59IndustryLoader,
    v59_industry_filter,
    V59_INITIAL_CAPITAL, V59_MAX_POSITIONS,
    V59_WEEKLY_TRADE_LIMIT, V59_GLOBAL_TRADE_LIMIT,
    V59_ENTRY_TOP_N, V59_INDUSTRY_TOP_N,
    V59_COMMISSION_RATE, V59_MIN_COMMISSION,
    V59_SLIPPAGE_BUY, V59_SLIPPAGE_SELL,
    V59_STAMP_DUTY, V59_TRANSFER_FEE,
    V59_WASH_SALE_WINDOW, V59_FRICTION_COST,
    V59_MAX_ITERATION_ROUNDS, V59_RETURN_TARGET,
    V59_MDD_TARGET, V59_PROFIT_LOSS_RATIO_TARGET,
    V59_MOMENTUM_WEIGHT, V59_R2_WEIGHT,
    V59_RS_TOP_PERCENTILE, V59_VOLUME_BREAKOUT_MULT,
    V59_HARD_STOP_LOSS_ATR_MULT, V59_BREAKEVEN_PROFIT_THRESHOLD,
    V59_TRAILING_PROFIT_TRIGGER, V59_MAX_SINGLE_POSITION_PCT,
    V59_MA20_BREAKOUT
)


@dataclass
class V59AccountState:
    """V59 账户状态"""
    trade_date: str = ""
    total_equity: float = V59_INITIAL_CAPITAL
    available_cash: float = V59_INITIAL_CAPITAL
    position_market_value: float = 0.0
    daily_pnl: float = 0.0
    daily_pnl_ratio: float = 0.0
    total_pnl: float = 0.0
    total_pnl_ratio: float = 0.0
    peak_equity: float = V59_INITIAL_CAPITAL
...
class V59BacktestResult:
    ...
    winning_trades: int = 0
    losing_trades: int = 0
    final_equity: float = V59_INITIAL_CAPITAL
    initial_equity: float = V59_INITIAL_CAPITAL
    total_commission: float = 0.0
    total_slippage: float = 0.0
    total_stamp_duty: float = 0.0
    ...
    trades: List[V59Trade] = field(default_factory=list)
    positions: List[Dict] = field(default_factory=list)
    audits: List[V59TradeAudit] = field(default_factory=list)
    price_audit_violations: int = 0
    meets_target: bool = False
    target_analysis: str = ""


class V59BacktestEngine:
    """
    V59 回测引擎 - 完整执行引擎
    
    【核心功能】
    1. 动态止损（废除 8% 固定止损，使用 ATR 动态止损）
    2. 利润生长空间（浮盈<10% 不启动止盈/保本）
    3. 选股引擎 3.0（Volume_Breakout + 行业趋势过滤）
    4. 成交价审计（min(Trigger, Next_Open)）
    """
    
    def __init__(self, initial_capital: float = V59_INITIAL_CAPITAL):
        self.initial_capital = initial_capital
        self.account = V59AccountState(total_equity=initial_capital, available_cash=initial_capital, peak_equity=initial_capital)
        self.positions: Dict[str, V59Position] = {}
        self.trades: List[V59Trade] = []
        self.trade_audits: List[V59TradeAudit] = []
        self.wash_sale_records: List[V59WashSaleRecord] = []
        self.blacklist: Dict[str, V59BlacklistRecord] = {}
        self.weekly_counters: Dict[int, V59WeeklyTradeCounter] = {}
        self.equity_curve: List[float] = []
        self.drawdown_curve: List[float] = []
        self.trade_dates: List[str] = []
        self.price_audit_violations = 0
        
        self.factor_engine = V59FactorEngine()
        self.risk_manager = V59RiskManager()
    ...
    def run_backtest(self, factor_data: pl.DataFrame, price_data: pl.DataFrame,
        ...
        """运行完整回测"""
        try:
            logger.info("V59 Backtest Engine Starting...")
            ...
            logger.error(f"V59 Backtest FAILED: {e}")
            logger.error(traceback.format_exc())
            return {'error': str(e), 'result': None}
    
    def _reset_state(self):
        """重置状态"""
        self.account = V59AccountState(
    ...
    def _update_weekly_counter(self, trade_date: str):
        ...
        try:
            ...
            if week_key not in self.weekly_counters:
                self.weekly_counters[week_key] = V59WeeklyTradeCounter(
                    week_number=week,
                    year=year,
                    trade_count=0
                )
    ...
    def _execute_exit(self, trade_date: str, symbol: str, reason: str):
        ...
        sell_price = next_open * (1 - V59_SLIPPAGE_SELL)
        
        shares = position.shares
        sell_amount = sell_price * shares
        
        commission = max(sell_amount * V59_COMMISSION_RATE, V59_MIN_COMMISSION)
        stamp_duty = sell_amount * V59_STAMP_DUTY
        transfer_fee = sell_amount * V59_TRANSFER_FEE
        ...
        trade = V59Trade(
            ...
            slippage_applied=V59_SLIPPAGE_SELL,
    ...
    def _update_positions(self, trade_date: str, current_df: pl.DataFrame):
        ...
        for symbol, position in list(self.positions.items()):
            ...
            self.risk_manager.update_position_stops(
                position, current_price, current_atr, current_ma20
            )
    ...
    def _process_entries(self, trade_date: str, current_df: pl.DataFrame,
        ...
        if len(self.positions) >= V59_MAX_POSITIONS:
            return
        
        if self.global_trade_count >= V59_GLOBAL_TRADE_LIMIT:
            return
        
        weekly_counter = self.weekly_counters.get(self.current_year * 100 + self.current_week)
        if weekly_counter and weekly_counter.trade_count >= V59_WEEKLY_TRADE_LIMIT:
            return
        
        filtered_df, filter_stats = v59_industry_filter(
            factor_df, industry_data, self.industry_loader,
            trade_date, V59_INDUSTRY_TOP_N, index_data
        )
        
        candidates = filtered_df.sort('composite_score', descending=True).head(V59_ENTRY_TOP_N)
        ...
        for row in candidates.iter_rows(named=True):
            ...
            if V59_MA20_BREAKOUT and not ma20_breakout:
                continue
            
            composite_score = row.get('composite_score', 0)
            rs_rank = row.get('rs_rank', 9999)
            
            buy_price = current_price * (1 + V59_SLIPPAGE_BUY)
            ...
            max_position_value = self.account.available_cash * V59_MAX_SINGLE_POSITION_PCT
            ...
            commission = max(buy_amount * V59_COMMISSION_RATE, V59_MIN_COMMISSION)
            transfer_fee = buy_amount * V59_TRANSFER_FEE
            ...
            position = V59Position(
                ...
                hard_stop_price=buy_price - (V59_HARD_STOP_LOSS_ATR_MULT * current_atr),
            ...
            trade = V59Trade(
                ...
                amount=buy_amount,
                commission=commission,
                slippage=V59_SLIPPAGE_BUY,
                stamp_duty=0,
                transfer_fee=transfer_fee,
                total_cost=total_cost,
                reason=f"Entry Signal (Score={composite_score:.4f})",
                execution_price=buy_price,
                signal_date=trade_date,
                trigger_price=current_price,
                next_open_price=current_price,
                min_trigger_open=min(current_price, current_price),
                slippage_applied=V59_SLIPPAGE_BUY,
                price_audit_passed=True
            )
        ...
        for record in self.wash_sale_records:
            if record.symbol == symbol:
                try:
                    ...
                    if 0 < days_diff <= V59_WASH_SALE_WINDOW:
                        return True
                except Exception:
                    pass
        return False
    ...
    def _add_to_blacklist(self, symbol: str, trade_date: str, reason: str):
        ...
        try:
            current = datetime.strptime(trade_date, "%Y-%m-%d")
            expiry = current + timedelta(days=V59_WASH_SALE_WINDOW)
            
            self.blacklist[symbol] = V59BlacklistRecord(
                symbol=symbol,
                stop_date=trade_date,
                stop_reason=reason,
                blacklist_expiry_day=int(expiry.strftime("%Y%m%d")),
                days_remaining=V59_WASH_SALE_WINDOW
            )
        except Exception:
            pass
    ...
    def _check_compliance(self, trade_date: str):
        """检查合规性"""
        if self.global_trade_count > V59_GLOBAL_TRADE_LIMIT:
            logger.warning(f"COMPLIANCE VIOLATION: Global trade limit exceeded ({self.global_trade_count} > {V59_GLOBAL_TRADE_LIMIT})")
        
        weekly_counter = self.weekly_counters.get(self.current_year * 100 + self.current_week)
        if weekly_counter and weekly_counter.trade_count > V59_WEEKLY_TRADE_LIMIT:
            logger.warning(f"COMPLIANCE VIOLATION: Weekly trade limit exceeded ({weekly_counter.trade_count} > {V59_WEEKLY_TRADE_LIMIT})")
    ...
    def _generate_result(self) -> Dict[str, Any]:
        ...
        meets_target = (
            total_return >= V59_RETURN_TARGET and
            self.account.max_drawdown <= V59_MDD_TARGET and
            profit_loss_ratio >= V59_PROFIT_LOSS_RATIO_TARGET
        )
        
        target_analysis = []
        if total_return < V59_RETURN_TARGET:
            target_analysis.append(f"Return {total_return:.2%} < {V59_RETURN_TARGET:.1%}")
        if self.account.max_drawdown > V59_MDD_TARGET:
            target_analysis.append(f"MDD {self.account.max_drawdown:.2%} > {V59_MDD_TARGET:.1%}")
        if profit_loss_ratio < V59_PROFIT_LOSS_RATIO_TARGET:
            target_analysis.append(f"P/L {profit_loss_ratio:.2f} < {V59_PROFIT_LOSS_RATIO_TARGET:.1f}")
        
        result = V59BacktestResult(
            ...
            trade_dates=self.trade_dates.copy(),
            trades=self.trades.copy(),
            audits=self.trade_audits.copy(),
...
class MasterLoop:
    ...
    【核心逻辑】
    如果 Profit_Loss_Ratio < 2.0 或 Total_Return < 10%：
    - 强制动作 A：在下一轮尝试更换选股因子
    - 强制动作 B：调整头寸管理
    
    【自检要求】
    在生成报告前，必须对比至少 3 种完全不同的逻辑路径，选出表现最稳健的一组
    """
    
    def __init__(self, max_iterations: int = V59_MAX_ITERATION_ROUNDS):
        self.max_iterations = max_iterations
        self.iteration_results: List[V59IterationResult] = []
        ...
        self.current_parameters = {
            'momentum_weight': V59_MOMENTUM_WEIGHT,
            'r2_weight': V59_R2_WEIGHT,
            'entry_top_n': V59_ENTRY_TOP_N,
            'rs_top_percentile': V59_RS_TOP_PERCENTILE,
            'volume_breakout_mult': V59_VOLUME_BREAKOUT_MULT,
            'hard_stop_atr_mult': V59_HARD_STOP_LOSS_ATR_MULT,
            'breakeven_threshold': V59_BREAKEVEN_PROFIT_THRESHOLD,
            'trailing_profit_trigger': V59_TRAILING_PROFIT_TRIGGER
        }
    ...
    def run_iteration(self, factor_data: pl.DataFrame, price_data: pl.DataFrame,
        ...
        meets_target = (
            total_return >= V59_RETURN_TARGET and
            profit_loss_ratio >= V59_PROFIT_LOSS_RATIO_TARGET and
            max_drawdown <= V59_MDD_TARGET
        )
        
        iteration_result = V59IterationResult(
            iteration=iteration,
    ...
    def _adapt_strategy(self, total_return: float, profit_loss_ratio: float, max_drawdown: float):
        """根据结果调整策略"""
        if profit_loss_ratio < V59_PROFIT_LOSS_RATIO_TARGET:
            logger.info("强制动作 A: 盈亏比不达标，更换选股因子")
            self.current_logic_path += 1
            
            if self.logic_paths[self.current_logic_path % len(self.logic_paths)] == 'rsrs_momentum':
                self.current_parameters['momentum_weight'] = 0.5
                self.current_parameters['r2_weight'] = 0.5
            elif self.logic_paths[self.current_logic_path % len(self.logic_paths)] == 'volatility_quality':
                self.current_parameters['entry_top_n'] = 10
                self.current_parameters['rs_top_percentile'] = 0.20
        
        if total_return < V59_RETURN_TARGET:
            logger.info("强制动作 B: 收益率不达标，调整头寸管理")
            self.current_parameters['hard_stop_atr_mult'] = max(
                self.current_parameters['hard_stop_atr_mult'] - 0.2, 2.0
            )
            self.current_parameters['breakeven_threshold'] = max(
                self.current_parameters['breakeven_threshold'] - 0.02, 0.08
            )
        
        if max_drawdown > V59_MDD_TARGET:
            logger.info("风控调整：降低止损倍数")
            self.current_parameters['hard_stop_atr_mult'] = min(
                self.current_parameters['hard_stop_atr_mult'] + 0.3, 4.0
            )
    
    def get_best_result(self) -> Optional[V59IterationResult]:
        """获取最佳迭代结果"""
    ...
    def compare_logic_paths(self) -> Dict[str, Any]:
        ...
        best_path = max(comparison.keys(), key=lambda x: comparison[x]['stability_score'])
