"""
Final Strategy V2.4 - Iteration 25: 进攻性 Alpha 觉醒

【核心改进自 V2.3】
1. 信号锁定门槛下调 - 从 5% 降至 2.5%，让盈利单更快切换到 LGB 主导
2. 非线性权重映射 (Sigmoid-like) - 低波动时 LGB 权重固定 80%，高波动时线性滑动
3. 追踪止盈深度放宽 - 基础因子从 1.2 提升至 1.5，盈利>5% 时放松 2.0x + buffer 额外 0.5%
4. 因子层进化 - 新增 momentum_squeeze 因子，优化 relative_strength_sector

【V2.3 问题诊断】
- 信号锁定门槛过高 (5%)：导致大部分盈利单无法触发 100% LGB 切换
- 线性权重映射过于平滑：低波动市场未能给予 LGB 足够主导权
- 止盈放松不足：基础因子 1.2 无法适应高波动环境

【解决方案】
1. 降低信号锁定门槛至 2.5%：让盈利单更快享受 LGB 的非线性预测优势
2. Sigmoid-like 权重映射：
   - atr_rank < 0.4 (低波动): LGB=0.8, Ridge=0.2
   - atr_rank >= 0.4 (高波动): 线性滑动至 LGB=0.4, Ridge=0.6
3. 多级止盈放松：
   - 盈利>3%: relax_factor = 1.5
   - 盈利>5%: relax_factor = 2.0 + dynamic_buffer += 0.005
4. 新增 momentum_squeeze = Momentum_5 / Volatility_20，捕捉低波动上涨标的

作者：量化策略团队
版本：Final Strategy V2.4 (Iteration 25)
日期：2026-03-16
"""
...
# ===========================================
# 配置常量 - Iteration 25 更新
# ===========================================

# 【Iteration 25】多模型集成参数
ENSEMBLE_ENABLED = True
RIDGE_WEIGHT = 0.5      # Ridge 基础权重 50%
LGB_WEIGHT = 0.5        # LightGBM 基础权重 50%

# 【Iteration 25】动态权重调整参数 - 非线性 Sigmoid-like 映射
DYNAMIC_WEIGHT_ENABLED = True
ATR_VOLATILITY_WINDOW = 250  # 滚动窗口

# 非线性权重映射参数 (Sigmoid-like)
# 低波动 (atr_rank < 0.4): LGB 主导 (80%)
# 高波动 (atr_rank >= 0.4): 线性滑动
ATR_RANK_THRESHOLD = 0.4  # 低波动/高波动分界
LGB_WEIGHT_LOW_VOL = 0.8  # 低波动时 LGB 权重
RIDGE_WEIGHT_LOW_VOL = 0.2  # 低波动时 Ridge 权重
LGB_WEIGHT_HIGH_VOL_MIN = 0.4  # 高波动时 LGB 最低权重
RIDGE_WEIGHT_HIGH_VOL_MAX = 0.6  # 高波动时 Ridge 最高权重

# 【Iteration 25】信号锁定门槛下调 - 从 5% 降至 2.5%
SIGNAL_LOCK_ENABLED = True
SIGNAL_LOCK_PROFIT_THRESHOLD = 0.025  # V2.4: 2.5% 盈利阈值 (原 5%)

# 【Iteration 25】利润跑腾逻辑 - 深度放宽
TRAILING_STOP_RELAX_ENABLED = True

# 基础放松因子 - V2.4 从 1.2 提升至 1.5
TRAILING_STOP_PROFIT_THRESHOLD_L1 = 0.03  # 盈利>3%
TRAILING_STOP_RELAX_FACTOR_L1 = 1.5  # V2.4: 从 1.2 提升至 1.5

# 第二级：盈利>5% - V2.4 新增 0.5% buffer
TRAILING_STOP_PROFIT_THRESHOLD_L2 = 0.05  # V2.4: 从 6% 降至 5%
TRAILING_STOP_RELAX_FACTOR_L2 = 2.0  # V2.4: 从 1.5 提升至 2.0
TRAILING_STOP_BUFFER_EXTRA_L2 = 0.005  # V2.4 新增：额外 buffer
...
class Position:
    ...
    profit_relax_level: int = 0   # 放松级别 (0=未触发，1=L1, 2=L2)
    signal_locked: bool = False   # 是否已信号锁定 (100% LGB)
...
class TradeRecord:
    ...
    relax_level_triggered: int = 0  # 触发的放松级别
...
class FinalStrategyV24:
    """
    Final Strategy V2.4 - Iteration 25 进攻性 Alpha 觉醒
    
    核心改进:
        1. 信号锁定门槛下调至 2.5% - 让盈利单更快切换至 LGB 主导
        2. 非线性权重映射 (Sigmoid-like) - 低波动时 LGB=80%
        3. 追踪止盈深度放宽 - 基础因子 1.5，盈利>5% 时 2.0x + buffer+0.5%
        4. 因子增强 - momentum_squeeze + relative_strength_sector 优化
    """
    ...
    def __init__(
        ...
        logger.info("FinalStrategyV2.4 initialized")
    ...
    def _update_dynamic_weights(self, atr: float, symbol: str = None, df: pl.DataFrame = None) -> Tuple[float, float]:
        """
        【V2.4 非线性权重映射 - Sigmoid-like】
        
        【核心逻辑】
        - atr_rank < 0.4 (低波动): LGB=0.8, Ridge=0.2 (固定)
        - atr_rank >= 0.4 (高波动): 线性滑动，LGB 从 0.8 降至 0.4
        
        【金融逻辑】
        - 低波动市场：给非线性模型 (LGB) 绝对主导权，捕捉 Alpha
        - 高波动市场：适度增加 Ridge 权重，保持稳定性
        ...
        # 非线性权重映射 (Sigmoid-like)
        if atr_rank < ATR_RANK_THRESHOLD:
            # 低波动区域：LGB 主导
            ridge_weight = RIDGE_WEIGHT_LOW_VOL
            lgb_weight = LGB_WEIGHT_LOW_VOL
        else:
            # 高波动区域：线性滑动
            # atr_rank 从 0.4 到 1.0，lgb_weight 从 0.8 线性降至 0.4
            high_vol_range = 1.0 - ATR_RANK_THRESHOLD  # 0.6
            high_vol_position = (atr_rank - ATR_RANK_THRESHOLD) / high_vol_range
            lgb_weight = LGB_WEIGHT_LOW_VOL - (LGB_WEIGHT_LOW_VOL - LGB_WEIGHT_HIGH_VOL_MIN) * high_vol_position
            ridge_weight = 1.0 - lgb_weight
        
        # 确保权重在有效范围内
        ridge_weight = max(0.2, min(0.6, ridge_weight))
        lgb_weight = max(0.4, min(0.8, lgb_weight))
    ...
    def _get_dynamic_buffer(self, atr: float, relax_level: int = 0) -> float:
        """
        获取动态 Buffer - V2.4 增加 L2 级别额外 buffer
        ...
        # V2.4: L2 级别额外增加 0.5% buffer
        if relax_level >= 2:
            base_buffer += TRAILING_STOP_BUFFER_EXTRA_L2
    ...
    def predict_ensemble(self, df: pl.DataFrame, atr: float = 0.02, symbol: str = None) -> pl.DataFrame:
        """
        使用集成模型进行预测 - 【V2.4 关键修复】
        
        【修复内容】
        - 使用非线性映射 ridge_weight = f(atr_rank) with Sigmoid-like
        - 信号锁定门槛降至 2.5%：持仓盈利>2.5% 时 100% 采用 LGB
        """
        ...
        if SIGNAL_LOCK_ENABLED and symbol and symbol in self.positions:
            ...
            if profit_pct >= SIGNAL_LOCK_PROFIT_THRESHOLD:
                ...
                logger.debug(f"[信号锁定] {symbol}: 盈利={profit_pct:.1%} >= {SIGNAL_LOCK_PROFIT_THRESHOLD:.1%}, 100% LGB")
    ...
    def update_trailing_stop(self, symbol: str, position: Position, 
        ...
        【改进内容】
        - 基础放松因子从 1.2 提升至 1.5
        - 盈利>3%: relax_factor = 1.5 (L1)
        - 盈利>5%: relax_factor = 2.0 + dynamic_buffer += 0.005 (L2)
        """
        ...
        # L1: 盈利>3% - V2.4 放松因子从 1.2 提升至 1.5
        if TRAILING_STOP_RELAX_ENABLED and profit_pct >= TRAILING_STOP_PROFIT_THRESHOLD_L1:
            relax_factor = TRAILING_STOP_RELAX_FACTOR_L1  # 1.5
            relax_level = 1
            
            # L2: 盈利>5% - V2.4 放松因子从 1.5 提升至 2.0，额外增加 0.5% buffer
            if profit_pct >= TRAILING_STOP_PROFIT_THRESHOLD_L2:
                relax_factor = TRAILING_STOP_RELAX_FACTOR_L2  # 2.0
                relax_level = 2
                ...
                if position.profit_relax_level < 2:
                    ...
                    logger.info(f"[利润跑腾 L2] {symbol}: 盈利={profit_pct:.1%} >= {TRAILING_STOP_PROFIT_THRESHOLD_L2:.0%}, "
                               f"放松系数={relax_factor}, buffer+0.5%")
        ...
        if current_price < trailing_stop:
            relax_info = ""
            if relax_level == 1:
                relax_info = " (L1 放松 1.5x)"
            elif relax_level == 2:
                relax_info = " (L2 放松 2.0x+buffer)"
    ...
    def compute_relative_strength_sector(self, df: pl.DataFrame, period: int = 20) -> pl.DataFrame:
        ...
        # 限制相对强度在合理范围内
        relative_strength_clipped = relative_strength.clip(0.5, 2.5)  # V2.4 放宽上限至 2.5
    ...
    def compute_all_enhanced_factors(self, df: pl.DataFrame) -> pl.DataFrame:
        """计算所有增强因子 - 【V2.4 新增 momentum_squeeze】"""
        result = df.clone()
        ...
        result = self.compute_relative_strength_sector(result, period=20)  # V2.4 优化
        result = self.compute_momentum_squeeze(result)  # V2.4 新增
        return result
    ...
    def _get_financial_logic(self, factor_name: str) -> str:
        logic_map = {
            ...
            "return_20d": "20 日收益率",
            "momentum_squeeze": "动量挤压因子，捕捉低波动上涨标的",  # V2.4 新增
    ...
    def auto_adjust_params(self, backtest_result: BacktestResult, 
        ...
        # V2.4: 如果 trailing_stop 平均盈利 < 2%，说明止盈太紧
    ...
    def get_weight_trend_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "low_vol_days": sum(1 for _, _, _, rank in self.weight_history if rank < ATR_RANK_THRESHOLD),
            "high_vol_days": sum(1 for _, _, _, rank in self.weight_history if rank >= ATR_RANK_THRESHOLD),
        }
...
def run_walk_forward_validation(strategy: FinalStrategyV24) -> Dict[str, Any]:
    logger.info("=" * 60)
...
def generate_audit_report(strategy: FinalStrategyV24, walk_forward_result: Dict[str, Any],
                          stress_result: Dict[str, Any], attribution_result: Dict[str, Any],
                          liquidity_stress_result: Optional[Dict[str, Any]] = None) -> str:
    report = []
    report.append("# Iteration 25 全周期审计报告 (V2.4)")
    report.append("")
    report.append(f"**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"**审计版本**: Final Strategy V2.4 (Iteration 25)")
    report.append("")
    ...
    report.append("")
    report.append("Iteration 25 针对进攻性 Alpha 觉醒进行了全面升级:")
    report.append("")
    ...
    report.append("")
    report.append(f"1. **策略有效性**: Iteration 25 在盲测区间 (2024Q1-Q2) 实现 {bt_result.get('total_return', 0):.2%} 收益，")
    if bt_result.get('total_return', 0) > 0.0389:
    ...
    report.append(f"   低波动天数={weight_trend.get('low_vol_days', 0)}, 高波动天数={weight_trend.get('high_vol_days', 0)}")
    report.append("")
    ...
    report.append("")
    report.append(f"4. **新增因子**: momentum_squeeze 捕捉低波动上涨标的")
    report.append("")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Final Strategy V2.4 - Iteration 25")
    logger.info("进攻性 Alpha 觉醒")
    logger.info("=" * 60)
    
    strategy = FinalStrategyV24(
    ...
    report_path = Path("reports/Iteration25_V24_Audit_Report.md")
