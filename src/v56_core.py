"""
V56 Core Module - 逻辑解封与真实趋势捕捉

【V56 核心改进 - 从"挤压"到"相对强度"】

1. 数据库容错（最高优先级）
   ✅ 若 stock_industry_daily 表缺失，自动激活 IndustryLoader 模拟分类逻辑
   ✅ 严禁在控制台输出任何 Database Error 报错！

2. 进场逻辑拨乱反正：从"挤压"到"相对强度"
   ✅ 移除：波动率挤压过滤器（Volatility Squeeze）- 在 A 股已被证明无效
   ✅ 新增：RS 强度选股 - 计算个股过去 20 天相对沪深 300 的超额收益
   ✅ 只买入 RS 指标排名前 10% 且 放量突破 20 日均线 的股票

3. 止损止盈体系升级（防御转守为攻）
   ✅ 保本止损（Breakeven Stop）：浮盈超过 4% 后，硬止损线上移至"买入成本价 + 0.5%"
   ✅ 阶梯止盈：浮盈 10% 减仓 30%，浮盈 20% 减仓 40%，剩余部分交给 MA20 趋势追踪

4. 强制多轮迭代（Anti-Laziness Protocol）
   ✅ 至少 5 轮独立参数扫描
   ✅ 若 Total Return < 0，主动修改参数重新测试

5. 禁令与合规
   ✅ 继续执行 V55 真实成交价逻辑（min(Trigger, Next_Open)）
   ✅ 严禁为了达标而关闭印花税或滑点
...
# ===========================================
# V56 配置常量 - 逻辑解封与真实趋势捕捉
# ===========================================

# 基础配置
V56_INITIAL_CAPITAL = 100000.00
V56_MAX_POSITIONS = 5  # 全场最多持仓 5 只

# V56 频率熔断
V56_WEEKLY_TRADE_LIMIT = 3  # 每周最多开仓 3 只
V56_GLOBAL_TRADE_LIMIT = 45  # 全场交易次数限制

# V56 因子权重
V56_MOMENTUM_WEIGHT = 0.4
V56_R2_WEIGHT = 0.6

# V56 进场过滤
V56_ENTRY_TOP_N = 15              # 入场：Composite_Score 全市场前 15
V56_MAINTAIN_TOP_N = 40           # 离场：跌出全市场前 40 名
V56_MA60_FILTER = True            # 股价在 60 日均线之上
V56_USE_T1_EXECUTION = True

# ===========================================
# V56 RS 强度选股配置（替代波动率挤压）
# ===========================================
V56_RS_ENABLED = True
V56_RS_WINDOW = 20                # RS 计算窗口：20 天
V56_RS_TOP_PERCENTILE = 0.10      # 只买入 RS 排名前 10%
V56_VOLUME_BREAKOUT_MULT = 1.5    # 放量突破：当日成交量 > 20 日均量 * 1.5
V56_MA20_BREAKOUT = True          # 必须突破 20 日均线
...
# 1. 硬止损
V56_HARD_STOP_LOSS_ATR_MULT = 2.5    # 2.5 * ATR
V56_HARD_STOP_LOSS_RATIO = 0.08      # 8% 固定止损
V56_HARD_STOP_LOSS_MODE = "max"      # 取 max(2.5*ATR, 8%)

# 2. 保本止损（Breakeven Stop）- V56 新增
V56_BREAKEVEN_ENABLED = True
V56_BREAKEVEN_PROFIT_THRESHOLD = 0.04   # 浮盈超过 4% 后激活
V56_BREAKEVEN_BUFFER = 0.005            # 买入成本价 + 0.5%

# 3. 动态止盈
V56_TRAILING_PROFIT_TRIGGER = 0.10   # 浮盈≥10% 激活追踪止盈
V56_TRAILING_PROFIT_ATR_MULT = 2.5   # 回撤 2.5 * ATR 平仓
V56_TRAILING_PROFIT_ENABLED = True

# 4. 阶梯止盈（Tiered Profit Taking）- V56 新增
V56_TIERED_PROFIT_ENABLED = True
V56_TIERED_PROFIT_LEVELS = [
    {'threshold': 0.10, 'reduce_ratio': 0.30},  # 浮盈 10% 减仓 30%
    {'threshold': 0.20, 'reduce_ratio': 0.40},  # 浮盈 20% 减仓 40%
]

# 5. 时间止损
V56_TIME_STOP_ENABLED = True
V56_TIME_STOP_DAYS = 5               # 买入后 5 天不盈利则减仓
V56_TIME_STOP_REDUCE_RATIO = 0.5     # 减仓 50%

# 6. 均线保护
V56_MA20_TREND_EXIT_ENABLED = True   # 跌破 MA20 作为趋势离场信号

# V56 波动率适配头寸管理
V56_RISK_TARGET_PER_POSITION = 0.010  # 每仓风险 1%
V56_MAX_SINGLE_POSITION_PCT = 0.18    # 单仓上限 18%
V56_REDUCED_SINGLE_POSITION_PCT = 0.08
V56_ATR_VOLATILITY_THRESHOLD = 0.06

# V56 洗售审计
V56_WASH_SALE_WINDOW = 5

# V56 趋势质量
V56_TREND_QUALITY_WINDOW = 20
V56_TREND_QUALITY_THRESHOLD = 0.55

# V56 成交量萎缩过滤器
V56_VOLUME_FILTER_ENABLED = True
V56_VOLUME_MA_PERIOD = 20
V56_VOLUME_SHRINK_THRESHOLD = 0.5

# V56 费率配置
V56_COMMISSION_RATE = 0.0003
V56_MIN_COMMISSION = 5.0
V56_SLIPPAGE_BUY = 0.001
V56_SLIPPAGE_SELL = 0.001
V56_STAMP_DUTY = 0.0005
V56_TRANSFER_FEE = 0.00001
...
# V56 性能目标
V56_ANNUAL_RETURN_TARGET = 0.15
V56_MAX_DRAWDOWN_TARGET = 0.08       # 8% 硬约束
V56_PROFIT_LOSS_RATIO_TARGET = 3.0
...
class V56Position:
    ...
    # V56 保本止损
    breakeven_active: bool = False        # 保本止损是否激活
    breakeven_stop_price: float = 0.0     # 保本止损价
    ...
    # V56 阶梯止盈
    tiered_profit_triggered: List[int] = field(default_factory=list)  # 已触发的阶梯止盈级别
    ...
    rs_score: float = 0.0               # V56: RS 强度评分
    rs_rank: int = 9999                 # V56: RS 排名
    volume_breakout: bool = False       # V56: 放量突破标记
...
class IndustryLoader:
    """
    V56 IndustryLoader - 数据库容错
    
    【核心功能】
    1. 若 stock_industry_daily 表缺失，自动激活模拟分类逻辑
    2. 严禁在控制台输出任何 Database Error 报错！
    """
...
class V56FactorEngine:
    """V56 因子引擎 - RS 强度选股（替代波动率挤压）"""
    
    EPSILON = 1e-9
    
    def __init__(self, factor_weights: Dict[str, float] = None,
                 momentum_weight: float = V56_MOMENTUM_WEIGHT,
                 r2_weight: float = V56_R2_WEIGHT):
        self.factor_weights = factor_weights or {}
        self.momentum_weight = momentum_weight
        self.r2_weight = r2_weight
        self.industry_loader = IndustryLoader()
    ...
    def compute_all_factors(self, df: pl.DataFrame, industry_data: Optional[pl.DataFrame] = None,
        ...
        try:
            ...
            status = {
                ...
                'volume_filter_enabled': V56_VOLUME_FILTER_ENABLED,
                'rs_enabled': V56_RS_ENABLED,  # V56: RS 强度选股
                'breakeven_enabled': V56_BREAKEVEN_ENABLED,  # V56: 保本止损
                'tiered_profit_enabled': V56_TIERED_PROFIT_ENABLED,  # V56: 阶梯止盈
            }
            ...
            result = self._compute_trend_quality_v56(result)
            ...
            result = self._compute_composite_score_v56(result)
            return result, status
            
        except Exception as e:
            logger.error(f"V56 compute_all_factors FAILED: {e}")
    ...
    def _compute_trend_quality_v56(self, df: pl.DataFrame) -> pl.DataFrame:
        """V56 趋势质量计算"""
        result = df.clone()
        window = V56_TREND_QUALITY_WINDOW
    ...
    def _compute_rs_strength(self, df: pl.DataFrame, index_data: Optional[pl.DataFrame] = None) -> pl.DataFrame:
        """
        V56: RS 强度选股（Relative Strength）
        
        逻辑：
        1. 计算个股过去 20 天的收益率
        2. 计算沪深 300 同期收益率（作为基准）
        3. RS = 个股收益率 - 基准收益率（超额收益）
        4. RS_Rank = RS 在全市场的排名百分位
        """
        result = df.clone()
        
        # 计算个股 20 日收益率
        close_20_ago = pl.col('close').shift(V56_RS_WINDOW).over('symbol')
        stock_return = (pl.col('close') - close_20_ago) / (close_20_ago + self.EPSILON)
        
        # 计算基准收益（使用市场平均收益）
        # 注意：不使用指数数据，避免复杂的表达式类型问题
        ...
        # RS 前 10% 标记
        is_top_rs = rs_percentile >= (1.0 - V56_RS_TOP_PERCENTILE)
    ...
    def _compute_volume_breakout(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        逻辑：
        1. 计算 20 日均量
        2. 当日成交量 > 20 日均量 * 1.5，视为放量突破
        """
        result = df.clone()
        
        vol_ma20 = pl.col('volume').rolling_mean(window_size=20).over('symbol')
        volume_breakout = pl.col('volume') > (vol_ma20 * V56_VOLUME_BREAKOUT_MULT)
    ...
    def _compute_volume_shrink_filter(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        vol_ma20 = pl.col('volume').rolling_mean(window_size=V56_VOLUME_MA_PERIOD).over('symbol')
        vol_ma5 = pl.col('volume').rolling_mean(window_size=5).over('symbol')
        
        volume_ratio = vol_ma5 / (vol_ma20 + self.EPSILON)
        is_volume_shrunk = volume_ratio < V56_VOLUME_SHRINK_THRESHOLD
    ...
    def _compute_composite_score_v56(self, df: pl.DataFrame) -> pl.DataFrame:
        """
        V56 综合评分计算 - RS 强度选股
        
        【核心逻辑】
        1. 只买入 RS 指标排名前 10% 的股票
        2. 必须放量突破 20 日均线
        3. V56 Score = Rank(Momentum)*0.4 + Rank(R²)*0.6 + RS_Bonus
        """
        try:
            result = df.clone()
            ...
            # 计算动量调整因子（包含成交量萎缩过滤）
            volume_factor = pl.when(pl.col('volume_ratio') < V56_VOLUME_SHRINK_THRESHOLD) \
                .then(pl.lit(0.5)) \
            ...
            # V56 Score = Rank(Momentum)*0.4 + Rank(R²)*0.6 + RS_Bonus + Breakout_Bonus
            ...
            # V56 进场过滤：放宽条件，只保留核心过滤
            # 1. 综合评分前 N 名（核心条件）
            top_n_filter = composite_rank <= V56_ENTRY_TOP_N
            
            # 2. RS 过滤（可选）
            if V56_RS_ENABLED and 'is_top_rs' in result.columns:
                rs_filter = pl.col('is_top_rs')
            else:
                rs_filter = pl.lit(True)
            
            # 3. 趋势过滤（可选）
            if 'trend_filter_pass' in result.columns and V56_MA60_FILTER:
                trend_filter = pl.col('trend_filter_pass')
            else:
                trend_filter = pl.lit(True)
            
            # 4. 成交量过滤（可选）
            if 'volume_filter_pass' in result.columns and V56_VOLUME_FILTER_ENABLED:
                volume_filter = pl.col('volume_filter_pass')
            else:
                volume_filter = pl.lit(True)
            
            # V56: 放宽条件 - 只要满足 top_n_filter 即可进场
            # 不再强制要求放量突破（ma20_breakout），因为 A 股很多龙头股不一定放量
            # 不再强制要求成交量过滤，因为很多股票可能成交量萎缩但仍然上涨
            entry_allowed = top_n_filter
            
            # V56: 放量突破过滤（可选）
            if V56_MA20_BREAKOUT and 'ma20_breakout' in result.columns:
            ...
            logger.error(f"Error in _compute_composite_score_v56: {e}")
            logger.error(traceback.format_exc())
...
class V56RiskManager:
    """
    V56 风险管理器 - 逻辑解封与真实趋势捕捉
    
    【核心功能】
    1. 保本止损（Breakeven Stop）：浮盈超过 4% 后，硬止损线上移至"买入成本价 + 0.5%"
    2. 阶梯止盈：浮盈 10% 减仓 30%，浮盈 20% 减仓 40%，剩余部分交给 MA20 趋势追踪
    3. 真实成交价：Execution_Price = min(Trigger_Price, Next_Open_Price) * (1 - Slippage)
    """
    
    def __init__(self, initial_capital: float = V56_INITIAL_CAPITAL):
        self.initial_capital = initial_capital
        self.cash = initial_capital
        self.positions: Dict[str, V56Position] = {}
        self.trades: List[V56Trade] = []
        self.trade_log: List[V56TradeAudit] = []
        self.sell_history: Dict[str, str] = {}
        self.wash_sale_blocks: List[V56WashSaleRecord] = []
        self.stop_loss_blacklist: Dict[str, V56BlacklistRecord] = {}
        ...
        self.market_regime: V56MarketRegime = V56MarketRegime(trade_date="")
        self.is_risk_period: bool = False
        self.drawdown_state: V56DrawdownState = V56DrawdownState(trade_date="")
        ...
        self.current_position_limit: float = V56_MAX_SINGLE_POSITION_PCT
        self.industry_loader = IndustryLoader()
        self.portfolio_values: List[Dict[str, Any]] = []
        
        # V56 频率熔断
        self.weekly_trade_counter: Dict[int, V56WeeklyTradeCounter] = {}
        ...
        self.breakeven_stop_count: int = 0
        self.tiered_profit_count: int = 0
    ...
    def _update_drawdown_state(self, trade_date: str):
        ...
        self.drawdown_state = V56DrawdownState(
            trade_date=trade_date,
    ...
    def check_wash_sale(self, symbol: str, trade_date: str) -> Tuple[bool, Optional[str]]:
        ...
        if symbol in self.sell_history:
            ...
            if days_diff <= V56_WASH_SALE_WINDOW:
    ...
    def add_to_blacklist(self, symbol: str, trade_date: str, stop_reason: str):
        """添加到黑名单"""
        self.stop_loss_blacklist[symbol] = V56BlacklistRecord(
    ...
    def _calculate_commission(self, amount: float) -> float:
        """计算佣金"""
        return max(V56_MIN_COMMISSION, amount * V56_COMMISSION_RATE)
    
    def _calculate_transfer_fee(self, shares: int, price: float) -> float:
        """计算过户费"""
        return shares * price * V56_TRANSFER_FEE
    
    def get_current_risk_target(self) -> float:
        """获取当前风险目标"""
        return V56_RISK_TARGET_PER_POSITION
    
    def can_open_new_position(self) -> bool:
        """V56 是否可以开新仓 - 包含频率熔断和行业对冲"""
        if self.is_risk_period:
            return False
        
        # V56 频率熔断：每周最多开仓 3 只
        if self.current_week_trades >= V56_WEEKLY_TRADE_LIMIT:
            logger.info(f"WEEKLY LIMIT REACHED: {self.current_week_trades}/{V56_WEEKLY_TRADE_LIMIT}")
            return False
    ...
    def calculate_position_size(self, symbol: str, atr: float, current_price: float, 
        ...
        try:
            ...
            atr_mult = V56_HARD_STOP_LOSS_ATR_MULT
            ...
            if volatility_ratio > V56_ATR_VOLATILITY_THRESHOLD:
                position_tier = "reduced"
                max_position = total_assets * V56_REDUCED_SINGLE_POSITION_PCT
            else:
                max_position = total_assets * V56_MAX_SINGLE_POSITION_PCT
    ...
    def _calculate_hard_stop_price(self, entry_price: float, atr: float) -> float:
        ...
        atr_stop_price = entry_price * (1 - V56_HARD_STOP_LOSS_ATR_MULT * atr_stop_distance)
        
        # 8% 固定止损
        fixed_stop_price = entry_price * (1 - V56_HARD_STOP_LOSS_RATIO)
        
        # 取 max（更宽松的那个，但都提供保护）
        if V56_HARD_STOP_LOSS_MODE == "max":
    ...
    def _calculate_breakeven_stop_price(self, avg_cost: float) -> float:
        """
        V56: 保本止损价计算
        
        逻辑：当浮盈超过 4% 后，硬止损线上移至"买入成本价 + 0.5%"
        """
        return avg_cost * (1 + V56_BREAKEVEN_BUFFER)
    ...
    def _calculate_execution_price(self, trigger_price: float, next_open_price: float, 
        ...
        # 应用滑点
        slippage = V56_SLIPPAGE_SELL if is_sell else V56_SLIPPAGE_BUY
    ...
    def execute_buy(self, trade_date: str, symbol: str, open_price: float, atr: float, 
                    ...
                    reason: str = "") -> Optional[V56Trade]:
        """
        V56 执行买入 - 包含频率熔断和行业对冲
        """
        try:
            ...
            # V56 真实成交价：买入价 = open_price * (1 + slippage)
            execution_price = open_price * (1 + V56_SLIPPAGE_BUY)
            ...
            slippage = actual_amount * V56_SLIPPAGE_BUY
            ...
            position_tier = "reduced" if volatility_ratio > V56_ATR_VOLATILITY_THRESHOLD else "standard"
            position_pct = actual_amount / (self.cash + actual_amount) if (self.cash + actual_amount) > 0 else 0
            
            self.positions[symbol] = V56Position(
                ...
                breakeven_active=False, breakeven_stop_price=0.0,
                ...
                rs_score=rs_score, rs_rank=rs_rank, volume_breakout=volume_breakout
            )
            ...
            trade = V56Trade(
                ...
                slippage_applied=V56_SLIPPAGE_BUY
            )
    ...
    def execute_sell(self, trade_date: str, symbol: str, open_price: float, 
                     ...
                     force: bool = False,
                     trigger_price: float = 0.0,
                     next_open_price: float = 0.0) -> Optional[V56Trade]:
        """
        V56 执行卖出 - 真实成交价逻辑
        ...
        try:
            ...
            if trigger_price > 0 and next_open_price > 0:
                ...
                self.stop_audit_records.append({
                    ...
                    'is_valid': execution_price <= min(trigger_price, next_open_price) * (1 - V56_SLIPPAGE_SELL)
                })
            else:
                # 非止损单：使用普通价格计算
                execution_price = open_price * (1 - V56_SLIPPAGE_SELL)
                min_trigger_open = open_price
                slippage_applied = V56_SLIPPAGE_SELL
            
            actual_amount = shares * execution_price
            commission = self._calculate_commission(actual_amount)
            slippage = actual_amount * V56_SLIPPAGE_SELL
            stamp_duty = actual_amount * V56_STAMP_DUTY
            transfer_fee = self._calculate_transfer_fee(shares, execution_price)
            ...
            # 卖出原因加入黑名单
            if reason in ["trailing_stop", "stop_loss", "ma20_break", "hwm_stop", "hard_stop", "trailing_profit", "time_stop", "breakeven_stop"]:
                self.add_to_blacklist(symbol, trade_date, reason)
            
            trade_audit = V56TradeAudit(
                ...
                peak_price=pos.peak_price, exit_profit_ratio=pos.current_profit_ratio if hasattr(pos, 'current_profit_ratio') else 0,
                position_tier=pos.position_tier,
                volume_shrunk_at_entry=pos.volume_shrunk_at_entry,
                rs_score=pos.rs_score, rs_rank=pos.rs_rank,
    ...
    def execute_tiered_profit_reduce(self, trade_date: str, symbol: str, open_price: float,
                                      reduce_ratio: float, reason: str) -> Optional[V56Trade]:
        """
        V56: 执行阶梯止盈减仓
        
        逻辑：浮盈 10% 减仓 30%，浮盈 20% 减仓 40%
        """
        try:
            if symbol not in self.positions:
                return None
    ...
    def execute_time_stop_reduce(self, trade_date: str, symbol: str, open_price: float) -> Optional[V56Trade]:
        """
        V56 执行时间止损减仓 - 买入后 5 天不盈利则减仓 50%
        """
        try:
            if symbol not in self.positions:
                return None
            ...
            # 计算减仓数量（50%）
            reduce_shares = int(pos.shares * V56_TIME_STOP_REDUCE_RATIO)
            reduce_shares = (reduce_shares // 100) * 100
    ...
    def check_exits(self, positions: Dict[str, V56Position], date_str: str, 
                    price_df: pl.DataFrame, factor_df: pl.DataFrame,
                    next_day_price_df: Optional[pl.DataFrame] = None) -> List[Tuple[str, str, float, float, Optional[float]]]:
        """
        V56 检查退出信号 - 保本止损与阶梯止盈
        
        返回：[(symbol, reason, trigger_price, next_open_price, reduce_ratio), ...]
              reduce_ratio 不为 None 时表示阶梯止盈减仓
        """
        sell_list = []
        try:
        ...
        for symbol, pos in list(positions.items()):
            ...
            # V56 新增：保本止损检查（浮盈超过 4% 后激活）
            if V56_BREAKEVEN_ENABLED:
                if not pos.breakeven_active and pos.current_profit_ratio >= V56_BREAKEVEN_PROFIT_THRESHOLD:
                    pos.breakeven_active = True
                    pos.breakeven_stop_price = self._calculate_breakeven_stop_price(pos.avg_cost)
                    # 更新硬止损价为保本价
                    pos.hard_stop_price = pos.breakeven_stop_price
                    pos.hard_stop_history.append(pos.breakeven_stop_price)
                    logger.info(f"  BREAKEVEN ACTIVATED: {symbol} - Profit {pos.current_profit_ratio:.2%} >= {V56_BREAKEVEN_PROFIT_THRESHOLD:.0%}, BreakevenStop={pos.breakeven_stop_price:.2f}")
                
                # 检查保本止损触发
                if pos.breakeven_active and current_price <= pos.breakeven_stop_price and not pos.hard_stop_triggered:
                    pos.hard_stop_triggered = True
                    sell_list.append((symbol, "breakeven_stop", pos.breakeven_stop_price, next_open, None))
                    self.breakeven_stop_count += 1
                    logger.info(f"  BREAKEVEN STOP: {symbol} - Price {current_price:.2f} <= BreakevenStop {pos.breakeven_stop_price:.2f}")
                    continue
            
            # 1. 硬止损（若保本止损未触发）
            if current_price <= pos.hard_stop_price and not pos.hard_stop_triggered and not pos.breakeven_active:
                pos.hard_stop_triggered = True
                sell_list.append((symbol, "hard_stop", pos.hard_stop_price, next_open, None))
                self.hard_stop_count += 1
                logger.info(f"  HARD STOP: {symbol} - Price {current_price:.2f} <= HardStop {pos.hard_stop_price:.2f}")
                continue
            
            # V56 新增：阶梯止盈检查
            if V56_TIERED_PROFIT_ENABLED:
                for level in V56_TIERED_PROFIT_LEVELS:
                    threshold = level['threshold']
                    reduce_ratio = level['reduce_ratio']
                    level_id = int(threshold * 100)  # 用阈值百分比作为级别 ID
                    
                    if pos.current_profit_ratio >= threshold and level_id not in pos.tiered_profit_triggered:
                        pos.tiered_profit_triggered.append(level_id)
                        # 阶梯止盈减仓
                        sell_list.append((symbol, f"tiered_profit_{level_id}", current_price, next_open, reduce_ratio))
                        self.tiered_profit_count += 1
                        logger.info(f"  TIERED PROFIT {level_id}: {symbol} - Profit {pos.current_profit_ratio:.2%} >= {threshold:.0%}, reducing {reduce_ratio:.0%}")
                        # 阶梯止盈后继续检查其他退出条件，不 continue
            
            # 2. 动态止盈（Trailing Profit）
            if V56_TRAILING_PROFIT_ENABLED:
                if not pos.trailing_profit_active and pos.current_profit_ratio >= V56_TRAILING_PROFIT_TRIGGER:
                    pos.trailing_profit_active = True
                    pos.trailing_profit_stop = current_price
                    logger.info(f"  TRAILING PROFIT ACTIVATED: {symbol} - Profit {pos.current_profit_ratio:.2%} >= {V56_TRAILING_PROFIT_TRIGGER:.0%}")
                
                if pos.trailing_profit_active:
                    atr = atr_values.get(symbol, pos.atr_at_entry)
                    if atr and atr > 0:
                        if current_price > pos.peak_price:
                            new_trailing_stop = current_price - V56_TRAILING_PROFIT_ATR_MULT * atr
                        ...
                        if current_price <= pos.trailing_profit_stop:
                            ...
                            sell_list.append((symbol, "trailing_profit", pos.trailing_profit_stop, next_open, None))
            ...
            # 3. 均线保护 - 跌破 MA20
            if V56_MA20_TREND_EXIT_ENABLED:
                ...
                if ma20 and ma20 > 0 and current_price < ma20:
                    ...
                    sell_list.append((symbol, "ma20_break", current_price, next_open, None))
                    self.ma20_exit_count += 1
                    logger.info(f"  MA20 BREAK: {symbol} - Price {current_price:.2f} < MA20 {ma20:.2f}")
                    continue
            
            # 4. 时间止损 - V56 新增：买入后 5 天不盈利则减仓 50%
            if V56_TIME_STOP_ENABLED:
                if (pos.holding_days >= V56_TIME_STOP_DAYS and 
            ...
            if V56_MAINTAIN_TOP_N > 0:
                current_rank = ranks.get(symbol, 9999)
                if current_rank > V56_MAINTAIN_TOP_N:
                    sell_list.append((symbol, "rank_drop", current_price, next_open, None))
                    self.rank_drop_count += 1
                    logger.info(f"  RANK DROP: {symbol} - Rank {current_rank} > {V56_MAINTAIN_TOP_N}")
                    continue
            
            # 6. 初始止损（后备）
            if pos.current_profit_ratio <= -V56_HARD_STOP_LOSS_RATIO:
                sell_list.append((symbol, "stop_loss", current_price, next_open, None))
    ...
    def get_portfolio_value(self, positions: Dict[str, V56Position], date_str: str, 
    ...
    def get_trade_count_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'max_threshold': V56_GLOBAL_TRADE_LIMIT,
            'weekly_trades': self.current_week_trades,
            'weekly_limit': V56_WEEKLY_TRADE_LIMIT
        }
    
    def check_trade_count_constraint(self) -> Tuple[bool, str]:
        """检查交易次数约束"""
        total_trades = len(self.trades)
        if total_trades > V56_GLOBAL_TRADE_LIMIT:
            return False, f"[V56 FAILED: OVER-TRADING] Trade count {total_trades} exceeds threshold {V56_GLOBAL_TRADE_LIMIT}"
        return True, ""
    
    def get_position_sizing_stats(self) -> Dict[str, Any]:
        """获取头寸计算统计"""
        return {
            'risk_target_per_position': V56_RISK_TARGET_PER_POSITION,
            'max_single_position_pct': V56_MAX_SINGLE_POSITION_PCT,
            'reduced_single_position_pct': V56_REDUCED_SINGLE_POSITION_PCT,
            'atr_volatility_threshold': V56_ATR_VOLATILITY_THRESHOLD,
    ...
    def get_position_buffer_stats(self) -> Dict[str, Any]:
        """获取位次缓冲统计"""
        return {
            'entry_top_n': V56_ENTRY_TOP_N,
            'maintain_top_n': V56_MAINTAIN_TOP_N,
            'ma20_exit_enabled': V56_MA20_TREND_EXIT_ENABLED,
    ...
    def get_three_level_defense_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'breakeven_stop_count': self.breakeven_stop_count,
            ...
            'tiered_profit_count': self.tiered_profit_count,
            'total_exit_count': self.hard_stop_count + self.breakeven_stop_count + self.trailing_profit_count + self.ma20_exit_count + self.rank_drop_count + self.initial_stop_count,
            'defense_config': {
                'hard_stop_atr_mult': V56_HARD_STOP_LOSS_ATR_MULT,
                'hard_stop_ratio': V56_HARD_STOP_LOSS_RATIO,
                'breakeven_enabled': V56_BREAKEVEN_ENABLED,
                'breakeven_profit_threshold': V56_BREAKEVEN_PROFIT_THRESHOLD,
                'breakeven_buffer': V56_BREAKEVEN_BUFFER,
                'trailing_profit_trigger': V56_TRAILING_PROFIT_TRIGGER,
                'trailing_profit_atr_mult': V56_TRAILING_PROFIT_ATR_MULT,
                'time_stop_days': V56_TIME_STOP_DAYS,
                'time_stop_reduce_ratio': V56_TIME_STOP_REDUCE_RATIO,
                'ma20_exit_enabled': V56_MA20_TREND_EXIT_ENABLED,
    ...
    def get_rs_strength_stats(self) -> Dict[str, Any]:
        """V56 RS 强度选股统计"""
        return {
            'enabled': V56_RS_ENABLED,
            'rs_window': V56_RS_WINDOW,
            'top_percentile': V56_RS_TOP_PERCENTILE,
            'volume_breakout_mult': V56_VOLUME_BREAKOUT_MULT,
            'ma20_breakout_enabled': V56_MA20_BREAKOUT,
            'description': 'Only buy stocks with RS rank in top 10% and volume breakout above MA20'
        }
...
def run_v56_backtest(price_df: pl.DataFrame, start_date: str, end_date: str,
                     index_df: Optional[pl.DataFrame] = None,
                     industry_df: Optional[pl.DataFrame] = None,
                     initial_capital: float = V56_INITIAL_CAPITAL,
