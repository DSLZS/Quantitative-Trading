"""V172 Runner Script - IR Focus"""
import argparse
from src.alpha_research_v172 import V172Runner

def main():
    parser = argparse.ArgumentParser(description='Run V172 Audit')
    parser.add_argument('--year', type=int, default=2024, help='Year to audit')
    args = parser.parse_args()
    
    runner = V172Runner(output_dir='reports')
    result = runner.run_audit(args.year)
    
    print("\n" + "=" * 70)
    print(f"V172 Audit Summary - Year {args.year}")
    print("=" * 70)
