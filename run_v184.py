...
def run_v184_audit():
    """运行 V184 审计"""
    from src.alpha_research_v184 import V184BacktestRunner, TARGET_IC_2024, TARGET_IR_2024, TARGET_IC_2023, TARGET_IR_2023
    
    logger.info("=" * 70)
    logger.info("  V184 策略全量审计 (2023-2024)")
    logger.info("=" * 70)
...
""")
    ...
    json_path = Path('reports') / f"v184_audit_{timestamp}.json"
...
def main():
    """主函数"""
    try:
        exit_code = run_v184_audit()
        return exit_code
    except Exception as e:
        logger.error(f"V184 审计失败：{e}")
        import traceback
        traceback.print_exc()
        return 1
