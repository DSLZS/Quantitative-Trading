...
def run_v185_audit():
    """运行 V185 审计"""
    from src.alpha_research_v185 import V185BacktestRunner, TARGET_IC_2024, TARGET_IR_2024, TARGET_IC_2023, TARGET_IR_2023
    
    logger.info("=" * 70)
    logger.info("  V185 策略全量审计 (2023-2024)")
    logger.info("=" * 70)
...
""")
    ...
    json_path = Path('reports') / f"v185_audit_{timestamp}.json"
...
def main():
    """主函数"""
    try:
        exit_code = run_v185_audit()
        return exit_code
    except Exception as e:
        logger.error(f"V185 审计失败：{e}")
        import traceback
        traceback.print_exc()
        return 1
