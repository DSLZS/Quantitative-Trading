"""
V191 主运行脚本 - 三年全量回测 (2023-2025)

【任务目标】
1. 数据校验：确保 2023-2025 每日股票数 > 5000
2. 回测执行：基于 V190 逻辑执行三年全量回测
3. 验收指标：
   - 2023: IC > 0.08, IR > 0.50
...
# 导入模块
from alpha_model import get_alpha_model, AlphaModel

# 直接导入 engine.py (不是 src.engine 包)
...
def main():
    """主函数"""
    logger.info("=" * 70)
    logger.info("V191 三年全量回测 (2023-2025)")
    logger.info("=" * 70)
    ...
    # 执行跨年度审计
    logger.info("\n[Audit] Running cross-year audit...")
    results = engine.run_cross_year_audit(df, alpha_model, years)
    
    # 输出摘要
    logger.info("\n" + "=" * 70)
    logger.info("V191 Cross-Year Audit Summary")
    logger.info("=" * 70)
