...
def run_v187_audit():
    """运行 V187 审计"""
    from src.alpha_research_v187 import V187BacktestRunner, TARGET_IC_2024, TARGET_IR_2024, TARGET_IC_2023, TARGET_IR_2023
    
    logger.info("=" * 70)
    logger.info("  V187 策略全量审计 (2023-2024)")
    logger.info("=" * 70)
...
""")
    ...
    json_path = Path('reports') / f"v187_audit_{timestamp}.json"
...
def main():
    """主函数"""
    try:
        exit_code = run_v187_audit()
        return exit_code
    except Exception as e:
        logger.error(f"V187 审计失败：{e}")
        import traceback
        traceback.print_exc()
        return 1
